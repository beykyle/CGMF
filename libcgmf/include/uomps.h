...
/*****************************************************/
static string add_name[ADD_CUSTOM_OMP] = {"scratch", "Madland", "Efrem",
                                          "spline" /*, "your_func" */};

unsigned int add_library(int id, double e, int zt, int at, int zi, int ai,
                         Optical *omp) {
  unsigned int potfm;
  switch (id - N_OMPLIB) {
  case 1:
    potfm = scratch_omp(e, at, zt, ai, zi, omp);
    break;
  case 2:
    potfm = madland(e, at, zt, omp);
    break;
  case 3:
    potfm = soukhovitskii(e, at, zt, zi, omp);
    break;
  case 4:
    potfm = spline(e, at, zt, ai, zi, omp);
    break;
    /*****************************************************/
    /*     4) Add one line                               */
    /*****************************************************/
...
/*         User defined OMPs                         */
/*****************************************************/
unsigned int scratch_omp(double e, int at, int zt, int ai, int zi,
                         Optical *omp) {
  if (ai != 1)
    return (0);
  if (e < 0.0 || e > 100.0)
...
unsigned int soukhovitskii(double e, int at, int zt, int zi, Optical *omp) {
  double eps, ef, a13, ex, v0, v1, v2, va, vd, wsd, wsa, wvd, vso, wsod, wis,
      wiv, wi0, wiso, lmdv, lmds, lmdso, cvso, cwso, phi;
  ...
  phi = (zi == 0) ? 0.0
                  : (-v1 - 2 * v2 * ex + vd * lmdv * exp(-lmdv * ex)) *
                        (1.0 + ((zi == 1) ? 1 : -1) * cvso * eps /
                                   (v0 + va * (at - 232) + vd)) *
                        0.90 * (double)zt / a13;

  omp->v1 =
      (v0 + va * (at - 232) + v1 * ex + v2 * ex * ex + vd * exp(-lmdv * ex)) *
          (1.0 +
           ((zi == 1) ? 1 : -1) * cvso * eps / (v0 + va * (at - 232) + vd)) +
      phi;
  omp->v2 = 0.0;
  omp->v3 = 0.0;
...
unsigned int spline(double e, int at, int zt, int ai, int zi, Optical *omp) {
  ...
  fs.close();
#else
  spv.Setdat(0.0, 1.000);
  spw.Setdat(0.0, 1.000);
  spv.Setdat(5.0, 1.050);
  spw.Setdat(5.0, 1.050);
  spv.Setdat(10.0, 1.100);
  spw.Setdat(10.0, 1.100);
  spv.Setdat(15.0, 1.000);
  spw.Setdat(15.0, 1.000);
  spv.Setdat(20.0, 0.950);
  spw.Setdat(20.0, 0.950);
  emax = 20.0;
#endif
