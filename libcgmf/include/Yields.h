...
private:
  ...
  // There are now 4 sets of Gaussian parameters (for 1st, 2nd, 3rd, and 4th
  // chance fission) Each parameter 5 values (one for each Gaussian -- we note
  // that the light and heavy fragment Gaussians are degenerate)
  double w_a[4][5], w_b[4][5], w_c[4][5], sig_a[4][5], sig_b[4][5], sig_c[4][5],
      mu_a[4][5], mu_b[4][5], mu_c[4][5];
  bool NFsupp_YA[4]; // Lists if there are Gaussian parameters for each chance
                     // fission
  ...
  // There are 5 parameters for the <TKE>(En) parameterization -- assumes linear
  // relation (4 sets for 1st, 2nd, 3rd, and 4th chance fission)
  double tke_params[4][5];
  bool NFsupp_TKE[4];

  // There are 11 parameters for the power-law fits of <TKE>(Ah) and
  // sigma_TKE(Ah) (9 coefficients, the A-value we expand about, and the maximum
  // A)
  ...
  double averageTKE(double e); // Calculates average TKE for the system as a
