...
// static double multiChanceFissionProbabilities [MAX_ZAIDc][5][41]; // ZAID,
// 1st, 2nd, 3rd, 4h chance probabilities as a function of energy
...
struct RTAtype {
  int ZAID;
  double ratios[NUMA];
  RTAtype() { std::fill_n(ratios, NUMA, 1.0); }
};
...
public:
  ...
  void generateInitialFissionFragmentHistories(string outputFilename,
                                               int numberEvents);
  ...
  int ZAIDc0, Ac0, Zc0; // Original (before any pre-fission neutron emission)
                        // compound fissioning nucleus

  double incidentEnergy; // incident neutron energy [MeV] or equivalent incident
                         // neutron energy if a neutron is emitted
  double incidentEnergy0; // incident neutron energy [MeV] BEFORE any
...
private:
  ...
  //  void createDefaultInputFile (string inputFilename, int ZAID, double
  //  incidentEnergy, double alphaSpin);

  // To include with XML I/O
  //  void readXMLInputFile (string inputFilename);
  //	void createDefaultXMLInputFile (string inputFilename, int ZAID, double
  //incidentEnergy);
  ...
  void sampleFissionFragments(fissionFragmentType *lf, fissionFragmentType *hf,
                              int);
  void sampleFissionFragments(fissionFragmentType *lightFragment,
                              fissionFragmentType *heavyFragment, Yields *ffy);

  // << 1.0.7 >> added fragment momentum calculation
  void computeFragmentInitialConditions(double TKE, int Zl, int Al, int Zh,
                                        int Ah, double *Ul, double *Uh,
                                        double *spinl, double *spinh, int *parl,
                                        int *parh, double *energyRelease,
  ...
  double massExcessCompound; // excess mass in MeV for the compound fissioning
                             // nucleus
  double SnCompound; // neutron separation energy for the compound fissioning
                     // nucleus

  // << 1.0.6 >>

  double
      SnCompound0; // neutron separation energy for the original (before any
                   // pre-fission neutron emission) compound fissioning nucleus

  void
  constructYields(Yields *ffy); // constructs yields for multi-chance fission
  void cgmBinaryNeutron(int nemit,
                        int c0); // this is a wrapper similar to specMain
  ...
  void getPrefissionNeutronEnergy(
      double *); // int get_prefission_neutrons(double *);
  int getNumberPrefissionNeutrons(void);

  double *energy_grid1;
  double *sp_pfn;
  double cmEnergy_pfn[MAX_NUMBER_NEUTRONS_PRE];  // the energy of the generated
                                                 // prefission neutrons
  double labEnergy_pfn[MAX_NUMBER_NEUTRONS_PRE]; // energy of the boosted
                                                 // pre-fission neutrons
  int neutronType_pre[MAX_NUMBER_NEUTRONS_PRE];  // this is a placeholder, to be
                                                // used later to identify either
                                                // pre-equilibrium or
...
/* Charge distribution class - follows the Wahl coded systematics. All massess
 * are primary fragment masses */
class ZDistribution {
...
public:
   ...
   */
  void ComputeZDistribution(int, int, double, int, int, int, double *, double *,
...
private:
  ...
  /* Compound nucleus excitation energy separating low- from mid- from
   * high-energy fission */
  double PE_L, PE_H;
  ...
  /* Same as above but evaluated at the low- and high-energy boundaries - used
   * in the linear interpolation at mid-energy */
  ...
  /* Coefficients for the above parameters - see Tab. 2 (low-energy L) and 3
   * (high-energy H) */
