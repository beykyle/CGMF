...
/**********************************************************/
int omExternalFunction(int lmax, double rho_match, double coulomb,
                       double coulomb_scat0, Wavefunc *wfn) {
  ...
  if (coulomb == 0.0) {
    ...
    wfn->external[1].real = g1 = ((coulomb + 1.0 / rho_match) * p1 - p2) /
                                 (sqrt(1.0 + coulomb * coulomb));
    p2 = g1;
  }
  ...
  */
  ...
  do {
    ...
    for (l = 0; l <= lmax + 1; l++)
      wfn->external[l].imag = wfn->external[l].imag * a;
    for (l = 0; l <= lmax; l++) {
      wfn->extderiv[l].imag =
    ...
    if (l0 > MAX_L0)
      cgmTerminateCode("angular momentum too large", l0);
  } while (flag);
...
double omGfunction(int l, double eta, double rho, double g1, double g2) {
  ...
  return (((j + j1) * (eta + j * j1 / rho) * g2 -
           j1 * sqrt(j * j + eta * eta) * g1) /
...
/**********************************************************/
int omExternalClosed(int lmax, double rho_match, double coulomb,
                     Wavefunc *wfn) {
  double p1, p2, p3;
  ...
  if (coulomb == 0.0) {
    ...
    for (int l = 1; l <= lmax; l++) {
      wfn->extderiv[l].real =
          -(l * wfn->external[l].real + rho_match * wfn->external[l - 1].real);
