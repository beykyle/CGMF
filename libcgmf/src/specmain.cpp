...
static void specCompoundDecay(int, int, Transmission *, Transmission *,
                              double **, double **);
static double specTransmissionSum(Statcalcmode, bool *, int, int, int,
                                  double **, Transmission *, Transmission *,
                                  double, Nucleus *, double **);
static void specPopulationBypass(int k0, double dp, Nucleus *n, double *spc,
...
void specMain(double initJ, int initP, double spinf, double **spc,
  ...
  /*** find nearest discrete level for given Ex */
    ...
    nstart = statStoreLevelExcite(&ncl[0]);

  /*** Hauser-Feshbach calculation */
  if (BINARY_REACTION_ONLY) {
    if (ncl[0].ncont > 0)
      spectraBinary(0, pdt, spc);
    else
      specGammaDiscrete(nstart, spc[0], &ncl[0]);
  } else if (EVAPORATION_SPECTRUM) {
    if (ncl[0].ncont > 0)
      specEvaporation(nemit, pdt, spc);
  } else {
    if (ncl[0].ncont > 0)
      spectra(nemit + 1, pdt, spc);
    else
      specGammaCascade(nstart, spc[0], &ncl[0]);
  }
...
void spectra(int nc, Pdata *pdt, double **spc) {
  ...
  try {
    ...
    for (int k = 0; k < MAX_LEVELS; k++)
      td[k].tran = new double[3 * MAX_J];
    for (int i = 0; i < MAX_MULTIPOL; i++)
      tg[i] = new double[MAX_ENERGY_BIN];
    for (int i = 0; i < 2; i++)
  ...
  for (int c0 = 0; c0 < nc; c0++) {
    ...
    for (int k0 = 0; k0 < ncl[c0].ncont; k0++) {
      ...
      if (ncl[c0].cdt[neutron].status)
        statStoreDiscreteTransmission(c0, k0, &pdt[neutron], td);
...
void specCompoundDecay(int c0, int k0, Transmission *tc, Transmission *td,
  ...
  if (tstat[1] &&
      (ncl[c0].excitation[k0] < ncl[c0].cdt[neutron].binding_energy)) {
    tstat[1] = false;
  }
  ...
  if (IGNORE_CAPTURE_GAMMA_RAY > 0.0) {
    if ((c0 == 0) && tstat[1]) {
      double excut =
          ncl[c0].cdt[neutron].binding_energy + IGNORE_CAPTURE_GAMMA_RAY;
      if (ncl[c0].excitation[k0] > excut)
        tstat[0] = false;
    }
  }
  ...
  for (int j0 = i0; j0 < MAX_J * 2 + i0; j0 += 2) {
    ...
    for (int p0 = -1; p0 <= 1; p0 += 2) {
      ...
      double pop =
          (p0 == 1) ? ncl[c0].pop[k0][jdx].even : ncl[c0].pop[k0][jdx].odd;
      if (pop == 0.0)
        continue;
      ...
      tsum = specTransmissionSum(sumall, tstat, k0, p0, j0, tg, tc, td, 0.0,
                                 &ncl[c0], spc);
      ...
      specTransmissionSum(hauser, tstat, k0, p0, j0, tg, tc, td, dp, &ncl[c0],
                          spc);
    }
  }
}
...
/**********************************************************/
double specTransmissionSum(Statcalcmode mode, bool *tstat, int k0, int pcn,
                           int jcn, double **tg, Transmission *tc,
                           Transmission *td, double dpop, Nucleus *n0,
                           double **spc) {
  double tsum = 0.0;

  /*** Sum up gamma-ray transmissions */
  if (tstat[0])
    tsum = specTransitionGamma(mode, k0, pcn, jcn, tg, dpop, n0, spc[0]);

  /*** Sum up neutron transmissions */
  if (tstat[1]) {
    Nucleus *n1 = &ncl[n0->cdt[neutron].next];
    tsum += specTransitionParticle(mode, k0, pcn, jcn, tc, td, dpop, n0, n1,
                                   spc[1]);
  }
...
/**********************************************************/
/* Ionel, 8/29/2013 */
void specPopulationBypass(int k0, double dp, Nucleus *n, double *spc, int jspin,
                          int parity) {
...
int specFindEnergyBin(double e, double de) {
  ...
  if ((de * k <= (e + eps)) && (e < de * (k + 1)))
    return (k);

  cerr << "out of range " << k << " " << de * k << " " << e << " "
       << de * (k + 1) << endl;
  return (-1);
}
