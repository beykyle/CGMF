...
void boost(double fragmentMomentum[3], int Zf, int Af, int neutronMultiplicity,
           ...
           int gammaMultiplicity, double cmGammaEnergies[MAX_NUMBER_GAMMAS],
  ...
  double phi, cmTheta,
      cosCmTheta; // direction of neutron emission in c.m. of fragment
  ...
  // fragment direction, in reference frame of fragment (pfy2 and pfz2 should be
  // zero!)
  double pfx2 = sinthetaF * cos(phiF) * pfx + sinthetaF * sin(phiF) * pfy +
                costhetaF * pfz; // along e_r
  double pfy2 = costhetaF * cos(phiF) * pfx + sin(phiF) * costhetaF * pfy -
                sinthetaF * pfz;                    // along e_theta
  double pfz2 = -sin(phiF) * pfx + cos(phiF) * pfy; // along e_phi

  Mn = NEUTRONMASS * amuMeV;

  /* IS: replace the loop over emitted neutrons with the routine
  neutron_emission_boost()
  ...
  for (int i=0; i<neutronMultiplicity; i++) {
    ...
    // direction of neutron emission chosen isotropic in c.m. of fission
  fragment phi = twopi*rng_cgm(); cosCmTheta = 2.0*rng_cgm()-1.0; cmTheta =
  acos(cosCmTheta);
  ...
  */
  ...
  for (int n = 0; n < neutronMultiplicity; n++) {
    neutronEnergies[n] = neutron_emission_boost(
        Zf, a_c, fragmentMomentum, cmNeutronEnergies[n], neutronVelocities[n],
        cmNeutronVelocities[n], false, Pcostheta, Ppreeq);
...
double neutron_emission_boost(int Z_c, int A_c, double *p_c,
                              double cmNeutronEnergy, double *neutronVelocities,
                              double *cmNeutronVelocities, bool preeq_flag,
  ...
  if (preeq_flag) {
    ...
    cosTheta = ((x2 - x1) * rnum - y1 * x2 + y2 * x1) /
               (y2 - y1); // interpolate between tabulated CDF points
  } else {
  ...
  for (int i = 0; i < 3; i++) {
    neutronVelocities[i] =
        cmNeutronVelocities[i] + p_c[i] / M_c; // lab velocity
...
=============================================================================*/
void boost_pfn(
    int Z_c, int A_c, double En, double TKE, int A_l, int Z_l, int A_h, int Z_h,
    int neutronMultiplicity_pre, double anisotropyCoefficient, bool preeq_flag,
    double cmNeutronEnergies[MAX_NUMBER_NEUTRONS_PRE], // pre-fission neutron
                                                       // energies in the CM
                                                       // (input)
    double neutronEnergies[MAX_NUMBER_NEUTRONS_PRE],   // pre-fission neutron
                                                       // energies in the lab
    int neutronType_pre[MAX_NUMBER_NEUTRONS_PRE], // this is a placeholder, to
                                                  // be used later to identify
                                                  // either pre-equilibrium or
                                                  // statistical neutrons
    double cmNeutronVelocities[MAX_NUMBER_NEUTRONS_PRE][3],
    double neutronVelocities[MAX_NUMBER_NEUTRONS_PRE][3],
    double Pcostheta[THETA_STEPS][2], double Ppreeq[THETA_STEPS][2],
    double *lightFragmentMomentum, double *heavyFragmentMomentum) {
  double M_c, Mn; // compound nucleus mass and neutron masses
  double phi, cosTheta,
      sinTheta; // direction of neutron emission in c.m. of fragment
  double Ecm;   // center-of-mass neutron energy
  double rnum;  // random number, saved for comparison purpose
  double x1, x2, y1,
      y2; // interpolate between the tabulated values of Pcostheta

  double p_c[3];
  int a_c = A_c; // running compound system mass taking into account the neutron
                 // emission
  ...
  int n0 = 0;
  if (preeq_flag) {
    neutronEnergies[0] = neutron_emission_boost(
        Z_c, a_c, p_c, cmNeutronEnergies[0], neutronVelocities[0],
        cmNeutronVelocities[0], true, Pcostheta, Ppreeq);
    n0 = 1;
  }

  for (int n = n0; n < neutronMultiplicity_pre; n++) {
    neutronEnergies[n] = neutron_emission_boost(
        Z_c, a_c, p_c, cmNeutronEnergies[n], neutronVelocities[n],
        cmNeutronVelocities[n], false, Pcostheta, Ppreeq);
  ...
  // All the prefission neutrons emitted, now the light fragment is emitted
  // isotropically (unless anisotropy coefficient available in anisotropy.dat)
  // and the heavy-fragment momentum is calculated from the momentum
  // conservation
  ...
  double Pf0 =
      sqrt(2. * TKE_cm /
           (1. / M_l + 1 / M_h)); // magnitude of the momentum for each fragment
  phi = twopi * rng_cgm();

  // AL: Including anisotropy in the emission of the fission fragments, for
  // select nuclei
  if (anisotropyCoefficient == 1.0) {
    cosTheta = 2.0 * rng_cgm() - 1.0; // sample isotropic when
  } else {
    // sample from the cos(theta) CDF distribution calculated in
