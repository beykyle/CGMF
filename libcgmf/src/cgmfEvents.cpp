...
void set_rng(std::function<double(void)> rng) { rng_cgm = rng; }
void set_rng(double (*rng)(void)) { rng_cgm = rng; }
...
int MAX_J;          ///< Maximum number of angular momentum values in the spin
                    ///< distributions
...
double CONTINUUM_LOWER_CUT; ///< lower-energy threshold to eliminate very small
                            ///< gamma energies in the continuum
...
cgmEvent::cgmEvent(int isotope, double eng, double time) {
  ...
  double spinf =
      0.0;       // multiplication factor for initial spin distribution (sigma)
  ...
  if (ctl.calc_montecarlo)
    specMCMain(initJ, initP, targE, spinf, nsim, spc);
  else
    specMain(initJ, initP, spinf, spc, 0.0);

  /*** Spectrum in the LAB frame */
  if (targE > 0.0)
    cgmLabSpectrum(targE / targA, ncl[0].binwidth, spc);
  ...
  allocateMemory();
  if (photonNu > 0) {
    for (int ii = 0; ii < mcl_nlines; ii++)
      photonEnergies[ii] = mcl_glines[ii];
  }
  if (neutronNu > 0) {
    for (int ii = 0; ii < mcl_nlinesn; ii++)
      neutronEnergies[ii] = mcl_elinesn[ii];
  }
...
cgmfEvent::cgmfEvent(int isotope, double eng, double time, double timew,
  ...
  if (firstcalc) {
    ...
    firstcalc = false;
    if (timew > 0.0)
      set_time_coincidence_window(timew);
  }
  cgmAllocateMemory();
  ...
  if (not sf) {
    ...
    eventLF.KE = lf[0].KE;
    for (int i = 0; i < 3; i++)
      eventLF.preFragmentMomentum[i] = lf[0].preMomentum[i];
  ...
  if (preFissionNeutronNu > 0) { // save the momenta of the pre-fission neutrons in the lab frame
    for (int i = 0; i < preFissionNeutronNu; i++) {
      double vn2 = 0.;
      for (int k = 0; k < 3; k++)
        vn2 += ff->labVelocities_pfn[i][k] * ff->labVelocities_pfn[i][k];
 ...
 *****************************************************************/
cgmfYields::cgmfYields(int isotope, double eng, int nevents, string outfilename)
    : cgmfEvent() {
  ...
  //	of << lf[i].Z << " " << lf[i].A << " " << lf[i].KE << " " << lf[i].U <<
  //" " << lf[i].spin << " " << lf[i].parity << endl; 	of << hf[i].Z << " " <<
  // hf[i].A << " " << hf[i].KE << " " << hf[i].U << " " << hf[i].spin << " " <<
  // hf[i].parity << endl;
  // }
  ...
  for (int i = 0; i < nevents; i++) {
    ...
    of << hf[i].Z << " " << hf[i].A << " " << hf[i].KE << " " << hf[i].U << " "
       << hf[i].spin << " " << hf[i].parity << " ";
...
void cgmfEvent::checkInput(int isotope, double einc) {
  ...
  if (einc == 0.0)
    isotope = -abs(isotope); // to ensure negative value

  int allowedIsotopes[] = {-94238, -94240, -94242, -94244, -98252,
                           -98254, 92233,  92234,  92235,  92238,
                           93237,  94239,  94241};
  int numIsotopes = sizeof(allowedIsotopes) / sizeof(int);

  bool found = false;
  for (int i = 0; i < numIsotopes; i++) {
    if (isotope == allowedIsotopes[i]) {
      found = true;
      break;
    }
  }
  ...
  if (not found) {
    cerr << "ERROR: Isotope not handled by CGMF or wrong incident energy\n";
    exit(-1);
  }
  if (isotope < 0 and einc != 0.0) {
    cerr << "ERROR: Only spontaneous fission is handled for this isotope\n";
    exit(-1);
  }
  if (isotope > 0 and (einc > 20.0 or einc <= 0.0)) {
    cerr << "ERROR: Incident neutron energy must be greater than 0 MeV and "
            "less than or equal to 20 MeV\n";
    exit(-1);
  }
...
void cgmfEvent::initialization() {
  ...
  //  riplReadDiscreteLevelData(); // read the entire RIPL3 database of discrete
  //  levels
  readDiscreteLevelData();
