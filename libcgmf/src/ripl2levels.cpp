...
void readDiscreteLevelData() {
  ...
  while (getline(fp, str)) { // loop over data file
    ...
    for (int i = 0; i < nol; i++) { // loop over discrete levels
      ...
      getline(fp, str);
      d = str.substr(0, 10);
      elev = atof(&d[0]); // energy (MeV)
      d = str.substr(10, 5);
      s = atof(&d[0]); // spin (hbar)
      d = str.substr(15, 3);
      p = atoi(&d[0]); // parity
      d = str.substr(18, 13);
      thlf = atof(&d[0]); // half-life (sec)
      d = str.substr(31, 3);
      ng = atoi(&d[0]); // number of gamma rays de-exciting the level
      ...
      for (int j = 0; j < ng; j++) {
        getline(fp, str);
        d = str.substr(0, 3);
        nf[j] = atoi(&d[0]); // serial number of the final state
        d = str.substr(3, 9);
        pe[j] = atof(&d[0]); // prob. of decay by electromagnetic transition
        d = str.substr(12, 10);
        ic[j] = atof(&d[0]);
      }
      ...
      /* moved this above next block, otherwise spin,parity of last level is set
       * to zero! */
      ...
      for (int j = 0; j < ng; j++) {
        if (j == 0) {
          ...
          allLevels[c][i].gratio[j] =
              1.0 / (1.0 + ic[j]); // Ig/(Ig + Ie) = 1/(1+ICC)
        } else {
          allLevels[c][i].fstate.push_back(nf[j]);
          allLevels[c][i].branch.push_back(pe[j]);
          allLevels[c][i].gratio.push_back(
              1.0 / (1.0 + ic[j])); // Ig/(Ig + Ie) = 1/(1+ICC)
        }
      }
...
/*!
 ...
 \todo I don't think I need this routine anymore once I finalize my new level
 file for CGMF.
...
void fixLevels(Nucleus *ncl, bool found, MaxLevelCtl ml) {
  ...
  if (!found) {
    ...
    if ((ncl[0].lev[0].spin < 0.0) || ncl[0].lev[0].parity == 0) {
      if (ml == reassign)
        ncl[0].ndisc =
            riplGSonly(ncl[0].za.getZ(), ncl[0].za.getA(), ncl[0].lev);
      else {
        os.str("");
        os << "discrete level data for Z " << ncl[0].za.getZ() << " - A "
...
int riplFixTransition(int nf[], double pe[], double ic[], int k, Level *lev) {
  ...
  /*! \bug Why choose the state with lowest energy? Wouldn't it be better to
   * choose the one closest in energy instead? */
  if (n == 0) {
    for (int i = 0; i < k; i++) {
      if ((s == 0.0) && (lev[i].spin == 0.0))
        continue;
...
int riplSetNmax(MaxLevelCtl ml, int nol, int nmax, int nc) {
  ...
  case reassign: // Nmax is used, and unknown level spin/parity will be
                 // re-assigned later
    nlevel = nmax;
    break;
  ...
  if (nlevel > MAX_LEVELS)
    nlevel = MAX_LEVELS; // cut at MAX_LEVELS
...
void riplReadDiscreteLevelData() {
  ...
  while (getline(fp, str)) { // loop over data file
    ...
    d = str.substr(5, 5);
    a = atoi(&d[0]); // mass number
    d = str.substr(10, 5);
    z = atoi(&d[0]); // charge number
    d = str.substr(15, 5);
    nol = atoi(&d[0]); // number of levels in decay scheme
    d = str.substr(
        20, 5); // nog  = atoi(&d[0]); // number of gamma rays in decay scheme
    d = str.substr(25, 5);
    nmax = atoi(&d[0]); // max. number of levels up to which the level scheme is
                        // complete
    d = str.substr(30, 5);
    nc = atoi(
        &d[0]); // level number up to which the spins and parities are unique
    ZAnumber za(z, a);
    ...
    for (int i = 0; i < nol; i++) { // loop over discrete levels
      ...
      for (int j = 0; j < ng; j++) {
        getline(fp, str);
        d = str.substr(39, 4);
        nf[j] = atoi(&d[0]) - 1; // serial number of the final state
        d = str.substr(66, 10);
        pe[j] = atof(&d[0]); // prob. of decay by electromagnetic transition
        d = str.substr(77, 10);
        ic[j] = atof(&d[0]);
      ...
      if (i < nlevel) { // store only data up to nlevel
        ...
        /* moved this above next block, otherwise spin,parity of last level is
         * set to zero! */
        allLevels[c][i].energy = elev;
        allLevels[c][i].spin = s;
        allLevels[c][i].halflife = thlf;
...
int riplReadDiscreteLevels(ZAnumber *za, Level *lev, MaxLevelCtl ml) {
  ...
  while (getline(fp, str)) {
    ...
    if (nmax >= MAX_LEVELS) {
      os.str("");
      os << "discrete levels for Z " << za1.getZ() << " - A " << za1.getA()
         << " too many (" << nol << ")";
      fp.close();
      cgmTerminateCode(os.str());
    }
      ...
      if (found && i < nlevel) {
        ...
        for (int j = 0; j < ng; j++) {
          if (j == 0) {
            ...
            lev[i].gratio.push_back(1.0 /
                                    (1.0 + ic[j])); // Ig/(Ig + Ie) = 1/(1+ICC)
          }
        }
      }
    }
  ...
  if (!found) {
    if (ml != reassign) {
      os.str("");
      os << "discrete level data for Z " << za->getZ() << " - A " << za->getA()
         << " not found";
      cgmTerminateCode(os.str());
    } else {
      nlevel = riplGSonly(za->getZ(), za->getA(), lev);
    ...
    if ((lev[0].spin < 0.0) || lev[0].parity == 0) {
      if (ml == reassign)
        ...
        os.str("");
        os << "discrete level data for Z " << za->getZ() << " - A "
           << za->getA() << " found but spin/parity not assigned";
        cgmTerminateCode(os.str());
      }
    }
  }
