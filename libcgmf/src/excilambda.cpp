...
double preqLambdaZeroZ(Preeq *q, Exconf *e) {
  ...
  lambda +=
      (densinteg(e1, q, &xw, &xp, &xr) + densinteg(e2, q, &xw, &xp, &xr)) * f0;
...
double epauli(int dzp, int dzh, int dnp, int dnh, Exconf *e, Preeq *q) {
  ...
  double z = preqPauliCorrection(e->zp + dzp, e->zh + dzh, e->np + dnp,
                                 e->nh + dnh, q->spd[0].gz, q->spd[0].gn);
  return (z);
}
