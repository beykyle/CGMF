...
static double specTc2c(Statcalcmode, int, int, int, int, int, double, double,
                       Nucleus *);
static double specTc2d(Statcalcmode, int, int, int, int, int, double, double,
                       Nucleus *);
...
double specTc2c(Statcalcmode mode, int pg, int k, int p0, int j0, int l,
  ...
  for (int j1 = j1min; j1 <= j1max; j1 += 2) {
    if (j0 == 0 && j1 == 0)
      continue; // gamma decay prohibited for 0->0
    ...
    if (pg * p0 == 1) {
      ...
      x0 = tg * dx;
      if (mode == hauser)
        n->pop[k][idx].even += (x1 = x0 * q);
    } else {
      dx = n->density[k][idx].odd * n->binwidth[k];
      x0 = tg * dx;
      if (mode == hauser)
        n->pop[k][idx].odd += (x1 = x0 * q);
    }
    if (mode == sumall)
      sum += x0;
    else if (mode == hauser)
...
double specTc2d(Statcalcmode mode, int pg, int i, int p0, int j0, int l,
  ...
  if (j0 == 0 && (int)n->lev[i].spin == 0 && j0 == 0)
    return (sum);
