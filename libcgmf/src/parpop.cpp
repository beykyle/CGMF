...
double specTransitionParticle(Statcalcmode mode, int k0, int pcn, int jcn,
  ...
  for (int k1 = 0; k1 < n1->ndisc; k1++) {
    ...
    if (mode == hauser) {
      double ep = n0->excitation[k0] - n1->lev[k1].energy -
                  n0->cdt[neutron].binding_energy;
...
double specTransitionParticleCont(Statcalcmode mode, int k1, int pcn, int jcn,
                                  Transmission *tc, double q, Nucleus *n0,
                                  Nucleus *n1) {
  int jtop = 0;
  ...
  for (int lp = 0; lp <= 2 * tc->lmax; lp += 2) {
    int pl = ((lp / 2) % 2 == 1) ? -1 : 1;
    for (int sp = n0->cdt[neutron].spin2; sp >= -n0->cdt[neutron].spin2;
         sp -= 2) {
      int jp = lp + sp;
      if (jp < 0)
        continue;
      ...
      for (int j1 = j1min; j1 <= j1max; j1 += 2) {
        ...
        if (pl * pcn == 1) {
          ...
          x0 = tj * dx;
          if (mode == hauser)
            n1->pop[k1][idx].even += (x1 = q * x0);
        } else {
          dx = n1->density[k1][idx].odd * n1->binwidth[k1];
          x0 = tj * dx;
          if (mode == hauser)
            n1->pop[k1][idx].odd += (x1 = q * x0);
        }
        if (mode == sumall)
          sum += x0;
        else if (mode == hauser)
...
double specTransitionParticleDisc(Statcalcmode mode, int k1, int pcn, int jcn,
                                  Transmission *td, double q, Nucleus *n0,
                                  Nucleus *n1) {
