...
/**********************************************************/
void preqPOccupation(double **p, double **t1, double **t2, double **tzp,
                     double **tnp, double **tz0, double **tn0)

{
    ...
    for (int i = 0; i < MAX_PREEQ; i++) {
      for (int j = 0; j < MAX_PREEQ; j++) {
        ...
        if (j > 0) {
          ...
          if (i < MAX_PREEQ - 1)
            y2 = tz0[i + 1][j - 1] * t1[i + 1][j - 1];
        }

        if (i > 1 && j < MAX_PREEQ - 1)
          x3 = p[i - 2][j + 1] * tzp[i - 2][j + 1] * t2[i - 2][j + 1];
        if (j > 1 && i < MAX_PREEQ - 1)
          x4 = p[i + 1][j - 2] * tnp[i + 1][j - 2] * t2[i + 1][j - 2];

        double t = x1 + x2 + y1 * (x3 + x5) + y2 * (x4 + x6);
        if (t > 0.0 && abs(p[i][j] / t - 1.0) > eps)
          conv = false;
        p[i][j] = t;
      }
    }
  ...
  if (!conv)
    cout << "P calc. not converged" << endl;
}
