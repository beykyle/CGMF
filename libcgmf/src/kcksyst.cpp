...
int kckDataRead(ZAnumber *za, LevelDensity *ldp) {
  ...
  if (!fp)
    cgmTerminateCode("level density parameter file not found");
  ...
  while (getline(fp, str)) {
    ...
    if ((za1.getZ() == za->getZ()) && (za1.getA() == za->getA())) {
      ...
      d = str.substr(11, 13);
      ldp->pairing_energy = atof(&d[0]);
      d = str.substr(24, 13);
      ldp->shell_correct = atof(&d[0]);
      d = str.substr(37, 10);
      ldp->match_energy = atof(&d[0]);
      d = str.substr(47, 10);
      ldp->a = atof(&d[0]);

      if (ldp->match_energy > 0.0) {
        d = str.substr(57, 10);
        ldp->temperature = atof(&d[0]);
        d = str.substr(67, 10);
        ldp->E0 = atof(&d[0]);
      } else {
        d = str.substr(77, 10);
  ...
  if (!found) {
    ostringstream os;
    os << "level density parameter for Z " << za->getZ() << " - A "
       << za->getA() << " not found";
...
int readkcksystdat() {
  ...
  while (getline(fp, str)) {
    ...
    d = str.substr(11, 13);
    kcksyst[zaid][0] = atof(&d[0]); // pairing_energy
    d = str.substr(24, 13);
    kcksyst[zaid][1] = atof(&d[0]); // shell_correct
    d = str.substr(37, 10);
    kcksyst[zaid][2] = atof(&d[0]); // match_energy
    d = str.substr(47, 10);
    kcksyst[zaid][3] = atof(&d[0]); // a

    if (kcksyst[zaid][2] > 0.0) {
      d = str.substr(57, 10);
      kcksyst[zaid][4] = atof(&d[0]); // temperature
      d = str.substr(67, 10);
      kcksyst[zaid][5] = atof(&d[0]); // EO
    } else {
      d = str.substr(77, 10);
      kcksyst[zaid][4] = atof(&d[0]); // temperature
      d = str.substr(87, 10);
