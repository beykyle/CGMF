...
*/
void setrngdptr(double (*funcptr)(void)) { set_rng(funcptr); }

/*!
\brief Used to check the directory path containing auxiliary data files, usually
...
/*!
\brief Used to set the directory path containing auxiliary data files, usually
in the main driver file.
*/
void setdatapath(const char *datapath) { setdatapath(string(datapath)); }
...
void cgm(int dza, double dex, int *dmcl_nlines, double dmcl_glines[30],
  ...
  *dmcl_nlines = (*cgm_event).getPhotonNu();
  for (int ii = 0; ii < *dmcl_nlines; ii++)
    dmcl_glines[ii] = (*cgm_event).getPhotonEnergy(ii);
  *dmcl_nlinesn = (*cgm_event).getNeutronNu();
  for (int ii = 0; ii < *dmcl_nlinesn; ii++)
    dmcl_elinesn[ii] = (*cgm_event).getNeutronEnergy(ii);
}

/*!
\brief Instantiates a new element of the cgmfEvent class.
Given a charge (Z), mass (A), and incident particle energy (erg), it
instantiates a new cgmfEvent.
*/
...
/*!
\brief Generates pre-neutron emission fission yields in mass, charge, kinetic
energy, excitation energy, spin and parity.
...
*/
int cgmf_getnnu() { return cgmf_fevent->getNeutronNu(); }
...
/*!
\brief Returns the first (u) directional cosine in the laboratory frame of the
i^th emitted neutron
*/
...
/*!
\brief Returns the second (v) directional cosine in the laboratory frame of the
i^th emitted neutron
*/
...
/*!
\brief Returns the third (w) directional cosine in the laboratory frame of the
i^th emitted neutron
*/
...
/*!
\brief Returns the time of emission (age) since fission of the i^th emitted
neutron
*/
double cgmf_getntme(int i) { return cgmf_fevent->getNeutronAge(i); }

/*!
\brief Returns the gamma multiplicity of the fission event
*/
int cgmf_getgnu() { return cgmf_fevent->getPhotonNu(); }

/*!
\brief Returns the lab energy of the i^th emitted photon
*/
double cgmf_getgerg(int i) { return cgmf_fevent->getPhotonEnergy(i); }

/*!
\brief Returns the first (u) directional cosine in the laboratory frame of the
i^th emitted photon
*/
double cgmf_getgdircosu(int i) { return cgmf_fevent->getPhotonDircosu(i); }

/*!
\brief Returns the second (v) directional cosine in the laboratory frame of the
i^th emitted photon
*/
double cgmf_getgdircosv(int i) { return cgmf_fevent->getPhotonDircosv(i); }

/*!
\brief Returns the third (w) directional cosine in the laboratory frame of the
i^th emitted photon
*/
double cgmf_getgdircosw(int i) { return cgmf_fevent->getPhotonDircosw(i); }

/*!
\brief Returns the time of emission (age) since fission of the i^th emitted
photon
*/
double cgmf_getgtme(int i) { return cgmf_fevent->getPhotonAge(i); }
...
double cgmf_getlfkepost() {
  return cgmf_fevent->getLightFragmentKineticEnergyPost();
}
double cgmf_gethfkepost() {
  return cgmf_fevent->getHeavyFragmentKineticEnergyPost();
}
...
double cgmf_getprenerg(int i) {
  return cgmf_fevent->getPreFissionNeutronEnergy(i);
}
double cgmf_getprendircosu(int i) {
  return cgmf_fevent->getPreFissionNeutronDircosu(i);
}
double cgmf_getprendircosv(int i) {
  return cgmf_fevent->getPreFissionNeutronDircosv(i);
}
double cgmf_getprendircosw(int i) {
  return cgmf_fevent->getPreFissionNeutronDircosw(i);
}
...
double cgmf_getlfpremomentum_x() {
  return cgmf_fevent->getLightFragmentPreMomentumX();
}
double cgmf_gethfpremomentum_x() {
  return cgmf_fevent->getHeavyFragmentPreMomentumX();
}

double cgmf_getlfpremomentum_y() {
  return cgmf_fevent->getLightFragmentPreMomentumY();
}
double cgmf_gethfpremomentum_y() {
  return cgmf_fevent->getHeavyFragmentPreMomentumY();
}

double cgmf_getlfpremomentum_z() {
  return cgmf_fevent->getLightFragmentPreMomentumZ();
}
double cgmf_gethfpremomentum_z() {
  return cgmf_fevent->getHeavyFragmentPreMomentumZ();
}

double cgmf_getlfpostmomentum_x() {
  return cgmf_fevent->getLightFragmentPostMomentumX();
}
double cgmf_gethfpostmomentum_x() {
  return cgmf_fevent->getHeavyFragmentPostMomentumX();
}

double cgmf_getlfpostmomentum_y() {
  return cgmf_fevent->getLightFragmentPostMomentumY();
}
double cgmf_gethfpostmomentum_y() {
  return cgmf_fevent->getHeavyFragmentPostMomentumY();
}

double cgmf_getlfpostmomentum_z() {
  return cgmf_fevent->getLightFragmentPostMomentumZ();
}
double cgmf_gethfpostmomentum_z() {
  return cgmf_fevent->getHeavyFragmentPostMomentumZ();
}
