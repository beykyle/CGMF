...
/**********************************************************/
double gdrGammaTransmission(GammaProfile c, int l, double eg, GammaMultipol gm,
                            double a, double ex) {
  string xl = "  ";
...
/**********************************************************/
double gdrGeneralizedLorentzian(int l, double eg, GDR *gam, double a,
                                double ex) {
  double c, g, f, t;
  ...
  f = c * gam->getSigma() * gam->getWidth() * pow(eg, 2.0 - 2.0 * l) *
      (gdrProfileLorentzian(eg, gam->getEnergy(), g) +
       ((l == 1) ? 0.7 *
                       gdrTempDependGamma(0.0, gam->getEnergy(),
                                          gam->getWidth(), t) /
...
double gdrProfileLorentzian(double eg, double er, double w) {
  if (er == eg && eg * w == 0.0)
    return (0.0);
  return (eg * w /
          ((er * er - eg * eg) * (er * er - eg * eg) + eg * eg * w * w));
}
