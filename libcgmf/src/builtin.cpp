...
static string lib_name[N_OMPLIB] = {
    ...
    "Perey",   "Schwandt",  "Lemos",    "Nolte",  "Avrigeanu", "Avrigeanu2009",
    "TALYS-A", "Bojowald",  "An",       "Han",    "bound",     "test"};
...
unsigned int find_omp(string str) {
  ...
  if (k == 0) {
    if ((k = findname(add_name, &str[0], ADD_CUSTOM_OMP)) != 0)
      k += N_OMPLIB;
  }
#endif
  if (k == 0)
    fprintf(stderr, "ERROR     : OMP name [%s] not found\n", &str[0]);
  return (k);
}
...
/***********************************************************/
unsigned int omp_library(int id, int zt, int at, int zi, int ai, double e,
                         Optical *omp) {
  unsigned int potfm;
  switch (id) {
  case 1:
    potfm = wilmore_hodgson(at, ai, zi, omp);
    break;
  case 2:
    potfm = becchetti_greenlees(at, zt, ai, zi, omp);
    break;
  case 3:
    potfm = rapaport(e, at, zt, ai, zi, omp);
    break;
  case 4:
    potfm = walter_guss(e, at, zt, ai, zi, omp);
    break;
  case 5:
    potfm = ch89(e, at, zt, ai, zi, omp);
    break;
  case 6:
    potfm = koning_delaroche(e, at, zt, ai, zi, omp);
    break;
  case 7:
    potfm = perey(at, zt, ai, zi, omp);
    break;
  case 8:
    potfm = madland_schwandt(e, at, zt, ai, zi, omp);
    break;
  case 9:
    potfm = lemos(ai, zi, omp);
    break;
  case 10:
    potfm = nolte(at, zt, ai, zi, omp);
    break;
  case 11:
    potfm = avrigeanu(e, at, zt, ai, zi, omp);
    break;
  case 12:
    potfm = avrigeanu2009(e, at, zt, ai, zi, omp);
    break;
  case 13:
    potfm = talys_alpha(e, at, zt, ai, zi, omp);
    break;
  case 14:
    potfm = bojowald(e, at, zt, ai, zi, omp);
    break;
  case 15:
    potfm = an_haixia(at, zt, ai, zi, omp);
    break;
  case 16:
    potfm = han_yinlu(at, zt, ai, zi, omp);
    break;
  case 17:
    potfm = shell_model(zi, omp);
    break;
  case 18:
    potfm = test_omp(zi, omp);
    break;
  default:
    potfm = 0;
    break;
...
unsigned int becchetti_greenlees(int at, int zt, int ai, int zi, Optical *omp) {
  ...
  if (ai == 1) { /* neutron or proton */
    if (zi == 0) {
      ...
      omp->v1 =
          54.0 + 24.0 * eps + 0.4 * (double)zt / pow((double)at, 1.0 / 3.0);
      omp->wv1 = -2.7;
...
unsigned int rapaport(double e, int at, int zt, int ai, int zi, Optical *omp) {
  ...
  if (e <= 15.0) {
    omp->wv1 = 0.0;
    omp->wv2 = 0.0;
    omp->ws1 = 4.28 - 12.8 * eps;
    omp->ws2 = 0.4;
  } else {
    omp->wv1 = -4.3;
    omp->wv2 = 0.38;
    omp->ws1 = 14.0 - 10.4 * eps;
    omp->ws2 = -0.39;
  }
...
/***********************************************************/
unsigned int walter_guss(double e, int at, int zt, int ai, int zi,
                         Optical *omp) {
  double eps, a13;
  ...
  if (e <= 40.0) {
    if (zi == 0) {
      omp->v1 = 52.56 - 16.5 * eps;
      omp->v2 = -0.310 + 0.081 * eps;
    } else {
      omp->v1 = 52.56 + 16.5 * eps + 0.4 * (double)zt / a13;
      omp->v2 = -0.310 - 0.081 * eps;
    }
  } else {
    if (zi == 0)
      omp->v1 = 52.56 - 0.310 * 40.0 * (1.0 + log(e / 40.0)) -
  ...
  if (e <= 39.4) {
    ...
    omp->wv1 = -0.963 + 0.153 * e * (1.0 - 0.33 * log(e / 39.4));
    omp->wv2 = 0.0;
  }

  if (zi == 0)
    omp->ws1 = 10.85 - 14.94 * eps;
  else
    omp->ws1 = 10.85 + 14.94 * eps - 1.30;
  omp->ws2 = -0.157;
...
/*      KD2001                                             */
/***********************************************************/
unsigned int koning_delaroche(double e, int at, int zt, int ai, int zi,
                              Optical *omp) {
  ...
  omp->rwso = omp->rvso;
  omp->rc = (zi == 0) ? 0.0
                      : 1.198 + 0.697 * pow((double)at, -2.0 / 3.0) +
...
unsigned int nolte(int at, int zt, int ai, int zi, Optical *omp) {
  ...
  omp->v1 = 101.1 + 6.051 * zt / a13;
  omp->v2 = -0.248;
  omp->v3 = 0.0;
  omp->wv1 = 26.82 - 1.706 * a13;
  omp->wv2 = 0.006;
  omp->wv3 = 0.0;
  ...
  omp->r0 = 1.245;
  omp->a0 = 0.817 - 0.0085 * a13;
  omp->rv = 1.570;
  omp->av = 0.692 - 0.020 * a13;
  omp->rs = 0.0;
  omp->as = 0.0;
...
/***********************************************************/
unsigned int avrigeanu2009(double e, int at, int zt, int ai, int zi,
                           Optical *omp) {
  ...
  if (e < e3) {
    omp->v1 = 168.0 + 0.733 * zt / a13;
    omp->v2 = -2.64;
  } else {
    omp->v1 = 116.5 + 0.337 * zt / a13;
    omp->v2 = -0.453;
  }

  omp->wv1 = 2.73 - 2.88 * a13;
  omp->wv2 = 1.11;
  ...
  if (e < e1) {
    ...
    omp->ws1 = 22.2 + 4.57 * a13 - 7.446 * e2;
    omp->ws2 = 6.0;
  } else {
    omp->ws1 = 22.2 + 4.57 * a13;
    omp->ws2 = -1.446;
  }

  omp->r0 = (e < 25.0) ? 1.18 + 0.012 * e : 1.48;
  omp->a0 = (e < e2)
                ? 0.671 + 0.0012 * at + (0.0094 - 0.0042 * a13) * e2
...
/***********************************************************/
unsigned int talys_alpha(double e, int at, int zt, int ai, int zi,
                         Optical *omp) {
  Optical nopt, popt;
  ...
  omp->v1 = 2 * nopt.v1 + 2 * popt.v1;
  omp->v2 = 0.0;
  omp->v3 = 0.0;
  omp->wv1 = 2 * nopt.wv1 + 2 * popt.wv1;
  omp->wv2 = 0.0;
  omp->wv3 = 0.0;
  omp->ws1 = 2 * nopt.ws1 + 2 * popt.ws1;
  omp->ws2 = 0.0;
  omp->ws3 = 0.0;
...
unsigned int bojowald(double e, int at, int zt, int ai, int zi, Optical *omp) {
  ...
  omp->r0 = 1.180;
  omp->a0 = 0.636 + 0.035 * a13;
  omp->rv = 1.270;
  omp->av = 0.768 + 0.021 * a13;
...
unsigned int han_yinlu(int at, int zt, int ai, int zi, Optical *omp) {
  ...
  omp->v1 = 82.18 - 34.811 * sym + 1.058 * zt / a13;
  omp->v2 = -0.148;
  omp->v3 = -0.000886;

  omp->wv1 = -4.916 + 35.0 * sym;
  omp->wv2 = 0.0555;
  omp->wv3 = 0.0000442;

  omp->ws1 = 20.968 - 43.398 * sym;
  omp->ws2 = -0.0794;
