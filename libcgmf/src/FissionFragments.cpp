 ...
 *	-- cleaned up and implemented fission yields parameterization by P.
 *Jaffke
 */

/****** GENERAL TO DO'S */
// TODO: 1) SHOULD WE STANDARDIZE THE READ-IN FOR LEVELDENSITY, LDP, ETC. TO
// MATCH THINGS LIKE READLDP()?
//       2) REFORMAT THE TABLES SO ITS 1D IN ZAI AND THERE'S SOME MAP FROM ZAI
//       --> INDEX TO CONSERVE ARRAY SIZE 3) GENERAL CLEANING, REMOVING UNUSED
//       FUNCTIONS, SOME EXPLANATION IN SOME PLACES
...
FissionFragments::FissionFragments(int ZAID, double Einc, double alphaSpin) {
  ...
  incidentEnergy =
      Einc; // Incident energy (which may be changed by multi-chance fission)
  ...
  if (incidentEnergy > 0.0)
    SnCompound =
        -getMassExcess(ZAIDc) + getMassExcess(ZAIDc - 1) + getMassExcess(1);
 ...
 ******************************************************************************/
FissionFragments::FissionFragments(int ZAIDf, double excitationEnergy,
                                   string option) {
  if (option.compare("single") != 0) {
    cerr << "Wrong option for FissionFragments\n";
  }
...
void FissionFragments::setOptions(void) {
  ...
  recomputeYields =
      true; // Default is to construct the yields (instead of reading them)
  ...
  case 92238: // U238
    ...
    anisotropy_flag = true;
    if (incidentEnergy == 0.0) {
      cerr << "ERROR: Only neutron-induced fission allowed for U238!" << endl;
      exit(0);
    }
    break;
  ...
  case 94238: // Pu238
    if (incidentEnergy != 0.0) {
      cerr << "ERROR: Only spontaneous fission allowed for Pu238!" << endl;
      exit(0);
    } else {
      sf_flag = true;
    }
  ...
  case 94240: // Pu240
    if (incidentEnergy != 0.0) {
      cerr << "ERROR: Only spontaneous fission allowed for Pu240!" << endl;
      exit(0);
    } else {
      sf_flag = true;
    }
  ...
  case 94242: // Pu242
    if (incidentEnergy != 0.0) {
      cerr << "ERROR: Only spontaneous fission allowed for Pu242!" << endl;
      exit(0);
    } else {
      sf_flag = true;
    }
  ...
  case 94244: // Pu244
    if (incidentEnergy != 0.0) {
      cerr << "ERROR: Only spontaneous fission allowed for Pu244!" << endl;
      exit(0);
    } else {
      sf_flag = true;
    }
  ...
  case 98252: // Cf252
    if (incidentEnergy != 0.0) {
      cerr << "ERROR: Only spontaneous fission allowed for Cf252!" << endl;
      exit(0);
    } else {
      sf_flag = true;
    }
  ...
  case 98254: // Cf254
    if (incidentEnergy != 0.0) {
      cerr << "ERROR: Only spontaneous fission allowed for Cf254!" << endl;
      exit(0);
    } else {
      sf_flag = true;
    }
  ...
  /* NOT SUPPORTED */
  default:
    cerr << "ERROR: Cannot use CGMF (yet) for this isotope and/or incident "
            "energy"
         << endl;
    exit(0);
  }
...
void FissionFragments::readMainInputFile(string inputFilename) {
  ...
  if (!inputFile) {
    cerr << "[readMainInputFile] Cannot find main input file!\n";
    cerr << inputFilename << "\n";
    exit(-1);
  }
  ...
  while (!inputFile.eof()) {
    ...
    if (found != string::npos) {
      ...
      if (key == "incidentEnergy")
        incidentEnergy = atof(value.c_str());

      if (key == "yieldsOption")
        yieldsOption = value.c_str();
      if (key == "yieldsFilename")
        yieldsFilename = value.c_str();

      //      if (key == "TKEmin") TKEmin = atoi(value.c_str());
      //      if (key == "TKEmax") TKEmax = atoi(value.c_str());

      if (key == "Amin")
        Amin = atoi(value.c_str());
      if (key == "Amax")
        Amax = atoi(value.c_str());
      if (key == "Zmin")
        Zmin = atoi(value.c_str());
      if (key == "Zmax")
        Zmax = atoi(value.c_str());

      if (key == "dZ")
        dZ = atoi(value.c_str());
      if (key == "sigmaZ")
        sigmaZ = atof(value.c_str());

      if (key == "energySortingOption")
        energySortingOption = value;

      if (key == "RT")
        RT = atof(value.c_str());

      if (key == "alphaI")
        alphaI = atof(value.c_str());
    }
  }
...
// TO INCLUDE WITH XML I/O
/*******************************************************************************
 * First reads an XML input data file, and creates an instance of
 *FissionFragments. It then builds the fission fragment yields Y(A,Z,TKE).
 *
 * << XCode note >>
 *
...
/* void FissionFragments::readXMLInputFile (string inputFilename) {
 ...
 auto_ptr<ffInputType> mainInput (ffInput(inputFilename)); // parses XML data
 file using XSD C++ routines
  ...
  if (levelDensities != 0) {
    for (int i = Amin; i <= Amax; i++) {
      if (levelDensities[i] != 0) {
        for (int j = -dZ; j <= dZ; j++) {
          if (levelDensities[i][j + dZ] != 0)
            delete[] levelDensities[i][j + dZ];
        }
  ...
  if (levelDensityParameters != 0) {
    for (int i = Amin; i <= Amax; i++) {
      if (levelDensityParameters[i] != 0) {
        for (int j = -dZ; j <= dZ; j++) {
          if (levelDensityParameters[i][j + dZ] != 0)
            delete[] levelDensityParameters[i][j + dZ];
        }
...
// TODO: REFORMAT LEVELDENSITIES TO BE A 1D ARRAY OF ZAID? WITH SOME MAPPING TO
// CONSERVE SPACE? SIMILAR TO readTemperatures. (P.J.)
// -----------------------------------------------------------------------------------------------------------------------------
/*******************************************************************************
...
void FissionFragments::readLevelDensityTables(string datafile) {
  ...
  while (getline(data, str)) {
    ...
    if (str.find("#") == 0) {
      ...
      pos = (int)str.find("A=");
      A = atoi(str.substr(pos + 2, 3).c_str());
      ...
      getline(data, str);
      getline(data, str); // skip two lines
 ...
 */
  ...
  for (int i = Amin; i <= Amax; i++) {
    ...
    for (int j = -dZ1; j <= dZ1; j++) {
      ...
      nucleus.ndisc = riplReadDiscreteLevels(&nucleus.za, nucleus.lev,
                                             reassign); // Use reassign
      statFixDiscreteLevels(&nucleus);
      statSetupEnergyBin(&nucleus);
      statSetupLevelDensityParameter(&nucleus, &nucleus.ldp);
      for (int k = 0; k < NUME; k++) {
        levelDensities[i][j + dZ1][k] =
            ldLevelDensity(ldEnergyGrid[k], i, &nucleus.ldp);
      }
    }

    /* FOR SAVING THE TABLE
      out << "\n# A=" << i << " ; Zp=" << int(Z0[i]+0.5) << " ; dZ=" << dZ <<
      "\n"; out << "# U / Z = "; out << setw(10); for (int j=-dZ; j<=dZ; j++) {
      out << setw(10) << int(Z0[i]+0.5)+j; } out << endl << endl; for (int k=0;
      k<NUME; k++) { out << setw(10) << ldEnergyGrid[k]; for (int j=-dZ; j<=dZ;
      j++) { out << setw(10) << levelDensities[i][j+dZ][k];
          }
      out << "\n";
      }
    */
...
void FissionFragments::readLevelDensityParameterTables(string datafile) {
  ...
  while (getline(data, str)) {
    ...
    if (str.find("#") == 0) {
      ...
      pos = (int)str.find("A=");
      A = atoi(str.substr(pos + 2, 3).c_str());
      //      pos = (int) str.find("Zp="); Z  =
      //      atoi(str.substr(pos+3,3).c_str()); pos = (int) str.find("dZ=");
      //      deltaZ = atoi(str.substr(pos+3,2).c_str());
  ...
  */
  ...
  for (int i = Amin; i <= Amax; i++) {
    ...
    for (int j = -dZ1; j <= dZ1; j++) {
      ...
      statSetupLevelDensityParameter(&nucleus, &nucleus.ldp);
      for (int k = 0; k < NUME; k++) {
        levelDensityParameters[i][j + dZ1][k] =
            ldDensityParameter(ldEnergyGrid[k], i, &nucleus.ldp);
      }
    }
    ...
    /* FOR SAVING THE TABLE
      ...
      out << setw(10) << int(Z0[i]+0.5)+j; } out << endl << endl; for (int k=0;
      k<NUME; k++) { out << setw(10) << ldEnergyGrid[k]; for (int j=-dZ; j<=dZ;
      j++) { out << setw(10) << levelDensityParameters[i][j+dZ][k];
          }
          out << "\n";
      }
    */
 ...
 ******************************************************************************/
double FissionFragments::computeEnergyFromMaxEntropy(int Zl, int Al,
                                                     double TIXE) {
  ...
  for (kl = 1; kl <= maxIndex; kl++) {
    ...
    numerator +=
        Ul * levelDensities[Al][dZl][kl] *
        levelDensities[Ah][dZh][kh]; // TODO: Change to 1D array in ZAI? (P.J.)
...
void FissionFragments::studyEnergySorting() {
  ...
  for (i = Amin; i < Amax; i++) {
    for (int j = -dZ; j <= dZ; j++) {
      ldFile << "# [gnuplot #" << ++count << "] A=" << i
             << " ; Z=" << (int)floor(Z0[i] + j + 0.5) << " (j=" << j
             << ")\n\n";
      for (int k = 0; k < NUME; k++) {
        ldFile << ldEnergyGrid[k] << " " << levelDensities[i][j + dZ][k] << " "
               << log(levelDensities[i][j + dZ][k])
  ...
  for (int i = Amin; i <= Amax; i++) {
    for (int j = 0; j <= 2 * dZ; j++) {
      tempFile << "# [gnuplot #" << ++count << "] A=" << i
               << " ; Z=" << (int)floor(Z0[i] + j - dZ + 0.5) << "\n\n";
      tempFile << setprecision(3) << setiosflags(ios::scientific);
      for (int k = 0; k < NUME; k++) {
        tempFile << ldEnergyGrid[k] << " " << temperatures[i][j][k] << " "
                 << levelDensities[i][j][k] << "\n";
      }
      tempFile << "\n";
    }
  }
  ...
  for (int i = 70; i <= Asym; i++) {
    ...
    for (int i = 1; i <= 200; i++) {
      ...
      maxEntropyFile << TXE << " " << x << " "
                     << TXE / (1 + float(Ac - A1) / float(A1)) << "\n";
    }
    maxEntropyFile << "\n";
  }
...
void FissionFragments::generateSingleFissionFragments(
    fissionFragmentType *lightFragment, fissionFragmentType *heavyFragment,
    int ZAIDf, double Eexc, const int numberEvents) {
 ...
 ******************************************************************************/
void FissionFragments::generateInitialFissionFragmentHistories(
    string outputFilename, int numberEvents) {
  ...
  if (yieldsOption ==
      "Systematics") { // Use of fission fragment yield systematics
    ...
    for (int i = 1; i <= numberEvents; i++) { // << 1.0.6 >>
      ...
      if (lightFragment.U > 0.0 && heavyFragment.U > 0.0) {
        ...
        outputFile << setw(4) << lightFragment.Z << setw(4) << lightFragment.A
                   << setw(10) << lightFragment.KE << setw(8) << lightFragment.U
                   << setw(8) << lightFragment.spin << setw(3)
                   << lightFragment.parity << endl;

        outputFile << setw(4) << heavyFragment.Z << setw(4) << heavyFragment.A
                   << setw(10) << heavyFragment.KE << setw(8) << heavyFragment.U
                   << setw(8) << heavyFragment.spin << setw(3)
                   << heavyFragment.parity;
      }
    }
...
void FissionFragments::sampleFissionFragments(
  ...
  double KEl, KEh;
  int iTKE, iA, iZ;
  iTKE = iA = iZ = 0; // Index version of TKE, A, Z
  int j;
  int Al, Zl;
  int Ah, Zh;
  int iZmin, iZmax;
  ...
  do {
    ...
    // Check if the sampled yield is lower than the Y(A,TKE) for the sampled A
    // and TKE
    if (y <= YATKE[iA][iTKE]) {
      ...
      do {
        ...
        if (j >= 10000) {
          cerr << "Charge sampling exceeded!" << endl;
        }
    ...
    if (i >= 10000) {
      cerr << "[sampleFissionFragments] Sampling did not converge!" << endl;
    }
...
void FissionFragments::sampleFissionFragments(
  ...
  do {
    TKE = ffy->sampleTKE(
        Ah); // Sample the TKE value from a Gaussian with mean <TKE>(Ah) and
...
void FissionFragments::computeFragmentInitialConditions(
  ...
  *energyRelease = massExcessCompound - getMassExcess(1000 * Zl + Al) -
                   getMassExcess(1000 * Zh + Ah); // Q-value
  ...
  if (totalExcitationEnergy < 0) {
    ...
    *Uh = 0.0;
    ...
    computeFragmentExcitationEnergy(totalExcitationEnergy, Zl, Al, Zh, Ah, Ul,
                                    Uh, energySortingOption);
  ...
  // IS: new routine that computes the fragment momentum, as well as the
  // pre-fission neutron momenta in the CM and in the lab

  boost_pfn(Zc0, Ac0, incidentEnergy0, TKE, Al, Zl, Ah, Zh, Ac0 - Ac,
            anisotropyCoefficient, preeq_flag, cmEnergy_pfn, labEnergy_pfn,
            neutronType_pre, cmVelocities_pfn, labVelocities_pfn, Pcostheta,
            Ppreeq, lightFragmentMomentum, heavyFragmentMomentum);
  ...
  /*  IS:  this is replaced by the above routine
    ...
    ---------------------------------------- double Pf0 = sqrt(
    (2.0*TKE)/(1.0/Ml+1.0/Mh) );

    // choose a (random) direction for the light fragment first (isotropic
    emission) double phi = rng_cgm()*twopi; double costheta = 2.0*rng_cgm()-1.0;
 ...
 * fragment mass A.
 ******************************************************************************/
void FissionFragments::sortExcitationEnergy(double Utot, int Al, int Ah,
                                            double *Ul, double *Uh, double RT) {
  ...
  do {
    il++;
  } while (ldEnergyGrid[il] <
           El); // Find the energy index corresponding to this split
  int ih = il;

  // Iteratively adjust El and Eh (and il and ih) until this energy ratio
  // matches RT = [El*ahf(Eh)]/[Eh*alf(El)]
  int count = 0;
  do {

    alf = levelDensityParameters[Al][dZ][il]; // Pull the level density
                                              // parameter for (Zl,Al) at energy
                                              // il -- assumes Zl = Zp(Al)
    ahf = levelDensityParameters[Ah][dZ][ih]; // Same for heavy fragment

    Eh = ahf / (alf * RT * RT + ahf) *
         Utot; // Calculate the Eh and El corresponding to the RT value
    El = Utot - Eh;
    ...
    do {
      il++;
             ...
             El); // Find the new light and heavy energy indices (il and ih)
    ih = 0;
    do {
      ih++;
    ...
    epsilon = abs(
        alf -
        levelDensityParameters[Al][dZ]
                              [il]); // Check if the sample El converged towards
                                     // the right alf -- works because a is
                                     // monotonically increasing

    // Don't sample more than 10 times
    if (++count > 10) {
      cerr << "[sortExcitationEnergy] did not converge!\n";
      exit(-1);
    }
 ...
 * energy sorting mechanism given by "sortingOption". << more info before
 *release >>
 ******************************************************************************/
void FissionFragments::computeFragmentExcitationEnergy(double TXE, int Zl,
                                                       int Al, int Zh, int Ah,
                                                       double *Ul, double *Uh,
                                                       string sortingOption) {

  // Intrinsic and collective excitation energy (used for sortingOption =
  // "maxEntropy"
  double
      Uint; // Total intrinsic excitation energy is shared between the fragments
  double Ucoll; // Total collective energy is the energy stored in the
                // collective degrees of freedom at scission (not shared)

  // TODO: THIS NEEDS TO BE ADJUSTED (BACK TO Z0INIT) IF ITS EVER GOING TO BE
  // USED (P.J.)
  int iZl =
      Zl; // - Z0init[Al] + dZ1; // Determine the index corresponding to Zl
  int iZh = Zh; // - Z0init[Ah] + dZ1; // and Zh

  if (sortingOption == "maxEntropy") {

    // TODO: IMPROVE THE MAX ENTROPY METHOD WITH THE DEFORMATION ENERGIES (WORK
    // WITH P. JAFFKE, P. TALOU, J. RANDRUP, AND R. VOGT)
    // -----------------------------------------------------------------------------------------------------------------------------

    // PROCESS:
    // 1) Somewhere before, read the deformation energies V_def(A,Z,U,epsilon)
    // -- maybe don't include U-dependence yet? 2) Calculate the total potential
    // energy at scission: V_tot = V_def(LF) + V_def(HF) + V_int(LF,HF) +
    // V_prox(LF,HF) for a variety of epsilon_LF and epsilon_HF values 3)
    // Determine the epsilon_LF and epsilon_HF that correspond to the minimum of
    // V_tot 4) Calculate V_int = TXE - V_def(LF,epsilon_LF) -
    // V_def(HF,epsilon_HF) and share V_int via max. entropy -- U_int(LF) +
    // U_int(HF) = V_int 5) Compute the total fragment excitation energies by
    // U_LF = U_int(LF) + V_def(LF,epsilon_LF) and U_HF = U_int(HF) +
    // V_def(HF,epsilon_HF) X? What to do about the fact that scission
    // deformation relaxes into g.s. deformation?
    ...
    if (ZAIDc == 98252) {
      if (abs(Ah - Asym) >=
          25) { // For more asymmetric fragments, assume less collective energy
        Ucoll = 0.3 * TXE;
      } else {
        Ucoll = 0.6 * TXE;
      }
    ...
    // Max. entropy principle states that the excitation energy is shared in the
    // configuration that contains the maximum entropy: S = log(rho) We can
    // solve for this configuration via <U_LF> = Int_0^Uint [e * rho(Zl,Al,e) *
    // rho(Zh,Ah,Uint-e)] de / Int_0^Uint [rho(Zl,Al,e) * rho(Zh,Ah,Uint-e)] de
    ...
    for (int k = 1; k <= NUME; k++) {
      ...
      if (ldEnergyGrid[k] > Uint) {
        break;
      } // Integrate only up to Uint
      ...
      // TODO: CHANGE LEVEL DENSITY TABLES FOR ZAI INSTEAD OF A AND Z.
      // SYNCHRONIZE THE ENERGY GRID WITH LDP? (P.J.)
      // -----------------------------------------------------------------------------------------------------------------------------
      y1 = log(levelDensities[Al][iZl][il - 1]); // S(E-dE)
      y2 = log(levelDensities[Al][iZl][il]);     // S(E)
      x1 = ldEnergyGrid[il - 1];                 // E-dE
      x2 = ldEnergyGrid[il];                     // E
      yl = (y2 - y1) / (x2 - x1) * (El - x1) +
           y1;      // S(El) = S(E-dE) + [S(E) - S(E-dE)]*[El - E + dE] / [dE]
  ...
  } else if (sortingOption == "fixedRT" || sortingOption == "Pfactors" ||
             sortingOption == "RTA") {
    // Everything else uses the ratio of temperatures

    double alf,
        ahf; // Level density parameters for the light and heavy fragment

    // TODO: PUT SOME EXPLANATION OF PFACTORS IN HERE - BASED ON CLOSED SHELLS
    // BLAH BLAH BLAH
    // -----------------------------------------------------------------------------------------------------------------------------
    ...
    do {
      ...
      Eh = ahf / (alf * RT * RT + ahf) *
           TXE; // Calculate the Eh and El corresponding to the RT value
      El = TXE - Eh;
      alf0 = alf;
      ...
      if (++count > 10) { // Cut off the convergence at 10 (rarely fails)
        cerr << "[computeFragmentExcitationEnergy] did not converge for ZAID: "
             << ZAIDl << ", " << ZAIDh << endl;
        //		exit(-1);
...
void FissionFragments::readRTAh(int Zt, int At) {
  ...
  static double U233_RTAh[43] = {
      // Adjusted to fit Nishio, 1998
      1.200, 1.200, 1.200, 1.200, 1.200, 1.200, 1.150, 1.150, 1.120,
      1.120, 1.200, 1.180, 1.180, 1.180, 1.180, 1.170, 1.110, 1.050,
      1.020, 1.100, 1.220, 1.230, 1.240, 1.240, 1.240, 1.240, 1.260,
      1.280, 1.280, 1.240, 1.240, 1.220, 1.170, 1.160, 1.160, 1.160,
      1.160, 1.160, 1.160, 1.160, 1.160, 1.160, 1.160};

  static double U235_RTAh[49] = {
      // Adjusted to fit Vorobyev, 2010
  ...
  static double U238_RTAh[49] = {
      // Old U235(nth,f) values copied for U238(n,f)
  ...
  static double Np237_RTAh[38] = {
      // Adjusted to fit Muller, 1981 (En = 0.8 MeV)
  ...
  static double Pu239_RTAh[51] = {
      // Adjusted to fit Apalin, 1965
      1.000, 1.130, 1.120, 1.160, 1.140, 1.190, 1.225, 1.240, 1.330,
      1.305, 1.260, 1.110, 1.105, 1.075, 1.020, 1.010, 1.000, 1.080,
      1.100, 1.075, 1.065, 1.045, 1.030, 1.015, 1.005, 0.985, 0.955,
      0.935, 0.910, 0.895, 0.875, 0.850, 0.825, 0.850, 0.830, 0.830,
      0.830, 0.830, 0.830, 0.830, 0.830, 0.830, 0.830, 0.830, 0.830,
      0.830, 0.830, 0.830, 0.830, 0.830, 0.830};

  static double Pu240_RTAh[51] = {
      // Adjusted by P. Jaffke to match Wahl nubar(A)
      1.300, 1.250, 1.150, 1.120, 1.150, 1.200, 1.230, 1.270, 1.300,
      1.310, 1.300, 1.270, 1.200, 1.100, 1.110, 1.270, 1.290, 1.300,
      1.290, 1.280, 1.270, 1.240, 1.210, 1.210, 1.250, 1.280, 1.300,
      1.320, 1.340, 1.350, 1.380, 1.400, 1.430, 1.450, 1.480, 1.480,
      1.440, 1.400, 1.380, 1.350, 1.330, 1.300, 1.250, 1.100, 1.000,
      0.980, 0.950, 0.900, 0.870, 0.850, 0.850};

  static double Pu241_RTAh[51] = {
      // Adjusted by P. Jaffke to match Wahl nubar(A)
      1.05,  1.111, 1.164, 1.212, 1.252, 1.287, 1.316, 1.339, 1.357,
      1.370, 1.378, 1.382, 1.381, 1.377, 1.369, 1.358, 1.344, 1.327,
      1.307, 1.285, 1.262, 1.237, 1.21,  1.182, 1.154, 1.125, 1.095,
      1.066, 1.037, 1.009, 0.981, 0.955, 0.93,  0.907, 0.885, 0.866,
      0.85,  0.836, 0.825, 0.818, 0.815, 0.815, 0.82,  0.829, 0.843,
      0.861, 0.886, 0.916, 0.951, 0.993, 1.041};

  static double Pu242_RTAh[51] = {
      // Adjusted by P. Jaffke to match Wahl nubar(A)
      1.128, 1.175, 1.217, 1.254, 1.286, 1.313, 1.335, 1.353, 1.367,
      1.377, 1.384, 1.387, 1.386, 1.383, 1.377, 1.368, 1.357, 1.343,
      1.328, 1.311, 1.292, 1.272, 1.251, 1.229, 1.206, 1.183, 1.16,
      1.136, 1.113, 1.09,  1.068, 1.046, 1.026, 1.007, 0.989, 0.973,
      0.959, 0.947, 0.937, 0.93,  0.925, 0.924, 0.926, 0.931, 0.94,
      0.952, 0.969, 0.99,  1.015, 1.045, 1.08};

  static double Cf252_RTAh[44] = {
      // Adjusted to fit Gook, 2014
      1.000, 1.173, 1.346, 1.519, 1.692, 1.613, 1.534, 1.455, 1.376,
      1.297, 1.307, 1.317, 1.328, 1.338, 1.348, 1.359, 1.328, 1.298,
      1.267, 1.237, 1.196, 1.166, 1.146, 1.115, 1.085, 1.054, 1.051,
      0.985, 0.980, 0.953, 0.977, 0.991, 0.996, 1.010, 1.024, 1.038,
      1.029, 1.020, 1.011, 1.002, 0.993, 0.984, 0.975, 0.496};

  static double Cf254_RTAh[43] = {
      // Same as Cf252(sf) but shifted
      1.173, 1.346, 1.519, 1.692, 1.613, 1.534, 1.455, 1.376, 1.297,
      1.307, 1.317, 1.328, 1.338, 1.348, 1.359, 1.328, 1.298, 1.267,
      1.237, 1.196, 1.166, 1.146, 1.115, 1.085, 1.054, 1.051, 0.985,
      0.980, 0.953, 0.977, 0.991, 0.996, 1.010, 1.024, 1.038, 1.029,
      1.020, 1.011, 1.002, 0.993, 0.984, 0.975, 0.496};
...
void readRTA(void) {
  ...
  if (!fp)
    cgmTerminateCode("R_T(A) data file not found");
...
void FissionFragments::getInitialSpin(int Z, int A, double *U, double *spin) {
  ...
  int j;
  double xj;
  double B2 = spinParameter2(
      *U, A,
      Z); // spin parameter B^2 (based on mom. of inertia and frag. deformation)

  oddSpin = 0.0;
  if (A % 2 != 0) {
    oddSpin = 0.5;
  } // For odd masses, add 1/2hbar
  ...
  for (int j = 0; j < numberSpins; j++)
    spinProba[j] /= sum; // Renormalize P(J)
...
void FissionFragments::buildYields(void) {
  ...
  SnCompound =
      -mass_excess(Zc, Ac) + mass_excess(Zc, Ac - 1) + mass_excess(0, 1);
  if (incidentEnergy == 0.0)
    SnCompound = 0.0; // SPONTANEOUS FISSION
  ...
  if (yieldsOption == "YATKE") {
    ...
    if (WahlParams.ReturnMethod() == "LegacyCGMF") {
      WahlParams.ComputeZDistribution(Zc, Ac, incidentEnergy + SnCompound, Amin,
                                      Amax, dZ, Z0, sZ0, FZZ0, FNZ0, &Zmin,
                                      &Zmax);
    } else {
      WahlParams.ComputeZDistribution(Zc0, Ac0, incidentEnergy0 + SnCompound0,
                                      Amin, Amax, dZ, Z0, sZ0, FZZ0, FNZ0,
                                      &Zmin, &Zmax);
    }
    ...
    // computeWahlParametersForChargeDistribution(); // Calculates the midpoint
    // Z0, width sZ0, and even-odd shell effects FNZ0, FZZ0 Set up discrete
    // levels, energy bins and level densities
    ncl[0].za.setZA(Zc, Ac);
    ncl[0].max_energy = incidentEnergy + SnCompound;
    nemit = statTotalNeutrons(
        ncl[0].max_energy,
        &ncl[0].za); // The maximum number of neutrons that can be emitted from
                     // (Zc,Ac) with energy Q = En + Sn

    // Read the pre-fission neutron emission probability for neutron-induced
    // fission
    if (sf_flag) {
      emissprob_max = 0.0;
    ...
    for (int i = 0; i <= nemit; i++) {
      ...
      statSetupLevelDensityParameter(
          &ncl[i],
          &ncl[i].ldp); // Get the level density parameters for this nucleus
    ...
    do {
      n_pfn = getNumberPrefissionNeutrons(); // sample the number of pre-fission
                                             // neutrons
    } while (n_pfn < 0);
    if (n_pfn == 0)
      preeq_flag = false; // catch for not sampling any pre-fission neutrons
    ...
    if (n_pfn > 0) {
      ...
      // double f_pe = 1.0/(1.0+
      // exp((12.4924628980014-incidentEnergy)/10.2096843664751))
      // -0.00422967345976505*incidentEnergy-0.249024419940462;
      if (f_pe < 0.)
        f_pe = 0.;
      ...
      if (preeq_flag) {
        ...
        evapInterface(pdt + neutron, tc, 0, 0,
                      sp1); // Create the evaporation spectrum
        ...
        excitonInterface(Zc, Ac - 1, incidentEnergy,
                         spc); // Calculate the pre-equilibrium spectrum
        for (int i = 0; i < MAX_ENERGY_BIN; i++) {
          sp1[i] =
              spc[neutron]
                 [i]; // now we can just sample from sp1 for pre-eq or compound
        }
        for (int j = 0; j < MAX_CHANNEL; j++) {
...
void FissionFragments::buildYZA(void) {
  ...
  for (i = Amin; i <= Amax; i++) {
    ...
    for (j = 0; j <= 2 * dZ; j++) {
      ...
      // Loop through the Z values for a given A (iZ is the true Z value and j
      // is the index)
      iZ = iZp0 + j - dZ;
    ...
    for (j = 0; j <= 2 * dZ; j++) {
      x += YZA[j][i];
    } // Find Sum of Y(Z|A) over Z -- NOT NECESSARILY 1 BECAUSE OF FINITE dZ
      // RANGE
    if (x != 0) {
      for (j = 0; j <= 2 * dZ; j++) {
        YZA[j][i] = YZA[j][i] * YA[i] /
                    x; // Calculate Y(Z,A) for a charge index j and mass i
      }
    }
...
void FissionFragments::buildYZ(void) {
  ...
  for (int i = Amin; i <= Amax; i++) {
    if (YA[i] != 0.0 && Z0[i] < Zpmin) {
      Zpmin = (int)floor(Z0[i] + 0.5);
    }
    if (YA[i] != 0.0 && Z0[i] > Zpmax) {
      Zpmax = (int)floor(Z0[i] + 0.5);
    }
  }

  // Validate Zpmin and Zpmax
  if (Zmin > Zpmin - dZ) {
    cout << "ERROR [buildYZ] Zmin > Zp(Amin)-dZ";
    exit(-1);
  }
  if (Zmax < Zpmax + dZ) {
    cout << "ERROR [buildYZ] Zmax < Zp(Amax)+dZ";
    exit(-1);
  }

  // Loop over the Zmin to Zmax (determined from
...
void FissionFragments::buildYATKEfromSystematics(void) {
  ...
  for (int i = Amin; i <= Amax; i++) {
    if (YA[i] != 0.0) {
      for (int j = TKEmin; j <= TKEmax; j++) {
        YATKE[i][j] =
            YA[i] * exp(-pow(j - meanTKE[i], 2) / (2 * pow(sigTKE[i], 2)));
      }
    }
  }
}
...
void FissionFragments::buildWahlYA(void) {
  ...
  int stop;
  int D1, F, G, H, J, K, M;
  double ECOR, NT1, Ip, Cp, Sp, Id, Cd, Sc, Dh, Sh, SYM, TH, Fz, Fn, nus1, Rf,
      Rj, sum, sum2;
  ...
  double Sn = -getMassExcess(1000 * Zc + Ac) +
              getMassExcess(Zc * 1000 + Ac - 1) + getMassExcess(1);
  ...
  if (Zt == 98 && Ac == 252) {
    NT1 = 2.286 + 0.147 * (Zt - 92) + 0.054 * (Ac - 236) +
          0.040 * (2 - Fz - Fn) + (0.145 - 0.0043 * (Ac - 236));
  } else {
    NT1 = 2.286 + 0.147 * (Zt - 92) + 0.054 * (Ac - 236) +
          0.040 * (2 - Fz - Fn) + (0.145 - 0.0043 * (Ac - 236)) * (En - TH);
  }
  ECOR = exp(-0.05 * PE);
  if ((NT1 + 1) > 4) {
    nus1 = NT1 + 1;
  ...
  while (stop == 0) {
    ...
    for (int i = Amin; i <= Amax; i++) {
      ...
      for (int j = 0; j < 7; j++) {
        Yield2[i] =
            Yield2[i] + Y[j] *
                            exp(-(pow(i - Nut[i] - Abar + delta[j], 2)) /
                                (2 * pow(sigma[j], 2))) /
                            (sigma[j] * sqrt(2 * 3.14159));
      }
    }
    for (int i = Amin; i < Amax; i++) {
      sum = sum + min(Yield2[i], Yield2[i + 1]) +
            abs(Yield2[i] - Yield2[i + 1]) / 2.0;
    }
...
void FissionFragments::buildSystematicsTKEvsMass(void) {
  ...
  P1 = 1.00 + 0.0112 * (Zc - 94) + 0.0009 * (Ac - 240) -
       0.00135 * pow(float(Zc - 94), 2);
  A3 = 15.20048736;
  A4 = -0.0798617477;
  A5 = 0.0001558700726;
  A1 = 39639.39298 * (1 + 0.000045 * (Zc - 94) + 0.000022 * (Ac - 240) -
                      0.0000018 * pow(float(Ac - 240), 2));
  A2 = -1272.328785 * (1 + 0.0000013 * (Zc - 94) + 0.0000006 * (Ac - 240) -
                       0.000000055 * pow(float(Ac - 240), 2));

  meanTKE[Asym] = A1 + A2 * Asym + A3 * pow(float(Asym), 2) +
                  A4 * pow(float(Asym), 3) + A5 * pow(float(Asym), 4);

  for (i = 1; i <= 15; i++) {
    meanTKE[Asym + i] = A1 + A2 * (Asym + i) + A3 * pow(float(Asym + i), 2) +
                        A4 * pow(float(Asym + i), 3) +
                        A5 * pow(float(Asym + i), 4);
  ...
  for (i = 21; i <= Amax - Asym; i++) {
    ...
    meanTKE[Asym - i] = meanTKE[Asym + i];
    if (Asym - i < 0) {
      cerr << "[buildSystematicsTKEvsMass] error; negative array index!\n";
      exit(-1);
    }
  }
  ...
  for (i = 16; i <= 20; i++) {
    meanTKE[Asym + i] =
        (meanTKE[Asym + 21] - meanTKE[Asym + 15]) / (21 - 15) * (i - 15) +
        meanTKE[Asym + 15];
    meanTKE[Asym - i] = meanTKE[Asym + i];
  }
}
 ...
 * generateInitialFissionFragmentHistories.
 ******************************************************************************/
void FissionFragments::checkDistributions(string inputFilename,
  ...
  if (!inputFile) {
    cerr << "[checkDistributions] ERROR: cannot find history file!" << endl;
    exit(-1);
  }
  double avTKE = 0.;
  ...
  while (!inputFile.eof()) {
    ...
    if (A >= Asym && !isSymmetric) {
      ...
      // Q-values
      Q = (mass_excess(Zc, Ac) - mass_excess(Zl, Al) - mass_excess(Zh, Ah) +
           incidentEnergy + SnCompound);
  ...
  // Mass Yields Y(A)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] Mass Yields Y(A)" << endl
             << endl;
  for (int i = 0; i < NUMA; i++) {
  ...
  // Charge Yields Y(Z)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] Charge Yields Y(Z)"
             << endl
             << endl;
  for (int i = 0; i < NUMZ; i++) {
  ...
  // TKE Yields Y(TKE)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] TKE   Y(TKE)  <Qf>(TKE)"
             << endl
             << endl;
  for (int i = 0; i < NUMTKE; i++) {
    if (numberEventsPerTKE[i] != 0) {
      ...
      outputFile << i << " " << tkeYields[i] << " " << QfTKE[i] << " "
                 << YTKEf[i] << endl;
    }
  }
  outputFile << endl;
  ...
  // P(U)
  outputFile << "# [gnuplot " << ++gnuplotIndex
             << "] Initial Excitation Energy Probability P(Ui)" << endl
             << endl;
  ...
  // <U>=f(A)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] <U>=f(A) " << endl
             << endl;
  for (int i = 0; i < NUMA; i++) {
    if (numberEventsPerMass[i] != 0) {
      excitationEnergies[i] /= numberEventsPerMass[i];
    }
  ...
  // <spin>=f(A)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] <spin>=f(A) " << endl
             << endl;
  for (int i = 0; i < NUMA; i++) {
    if (numberEventsPerMass[i] != 0) {
  ...
  // P(TXE)
  outputFile << "# [gnuplot " << ++gnuplotIndex
             << "] Total Excitation Energy Probability P(TXE)" << endl
             << endl;
  sum = 0;
  for (int i = 0; i < NUME; i++) {
    sum += initialTXE[i];
  ...
  // <TKE>(A)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] A   <TKE>(A) <Q>(A)"
             << endl
             << endl;
  double avTKE_ = 0.;
  ...
  for (int i = 0; i < NUMA; i++) {
    if (numberEventsPerMass[i] != 0) {
      ...
      sTKEvsMass[i] = sqrt(sTKEvsMass[i] / numberEventsPerMass[i] -
                           TKEvsMass[i] * TKEvsMass[i]);
      QfA[i] /= numberEventsPerMass[i];
      outputFile << setw(4) << i << " " << setw(6) << TKEvsMass[i] << " "
                 << setw(7) << right << sTKEvsMass[i] << " " << setw(6)
                 << QfA[i] << " " << numberEventsPerMass[i] << endl;
    }
  }
  outputFile << endl;
  ...
  // <TXE>(A)
  outputFile << "# [gnuplot " << ++gnuplotIndex << "] <TXE>=f(A) " << endl
             << endl;
  for (int i = 0; i < NUMA; i++) {
    if (numberEventsPerMass[i] != 0) {
  ...
  /* Uncomment for writeout of Y(A,TKE)
      ...
      for (int j=0; j<NUMTKE; j++) {
          ...
          if (sumYields!=0.0) {
              outputFile << "# [gnuplot " << ++gnuplotIndex << "] ; TKE = " << j
     << " " << sumYields << "\n\n"; for (int i=0; i<NUMA; i++) {
...
void FissionFragments::computeTemperatures(int Z, int A) {
  ...
  //    nucleus.ndisc =
  //    riplReadDiscreteLevels(&nucleus.za,nucleus.lev,reassign);
  ...
  for (int k = 2; k < NUME - 2; k++) {
    ...
    temp[k] = (-log(ld[k + 2]) + 8.0 * log(ld[k + 1]) - 8.0 * log(ld[k - 1]) +
               log(ld[k - 2])) /
              (12. * de);
  }

  de = ldEnergyGrid[1] - ldEnergyGrid[0];
  temp[0] = (-25.0 * log(ld[0]) + 48.0 * log(ld[1]) - 36.0 * log(ld[2]) +
             16.0 * log(ld[3]) - 3.0 * log(ld[4])) /
            (12.0 * de);

  de = ldEnergyGrid[2] - ldEnergyGrid[1];
  temp[1] = (-25.0 * log(ld[1]) + 48.0 * log(ld[2]) - 36.0 * log(ld[3]) +
             16.0 * log(ld[4]) - 3.0 * log(ld[5])) /
            (12.0 * de);

  de = ldEnergyGrid[NUME - 2] - ldEnergyGrid[NUME - 3];
  temp[NUME - 2] = (25.0 * log(ld[NUME - 2]) - 48.0 * log(ld[NUME - 3]) +
                    36.0 * log(ld[NUME - 4]) - 16.0 * log(ld[NUME - 5]) +
                    3.0 * log(ld[NUME - 6])) /
                   (12.0 * de);

  de = ldEnergyGrid[NUME - 1] - ldEnergyGrid[NUME - 2];
  temp[NUME - 1] = (25.0 * log(ld[NUME - 1]) - 48.0 * log(ld[NUME - 2]) +
                    36.0 * log(ld[NUME - 3]) - 16.0 * log(ld[NUME - 4]) +
                    3.0 * log(ld[NUME - 5])) /
                   (12.0 * de);
...
void FissionFragments::computeTemperatureTables(void) {
  ...
  for (int i = Amin; i <= Amax; i++) {
    ...
    for (int j = 0; j <= 2 * dZ1; j++) {
      ...
      for (int k = 2; k < NUME - 2; k++) {
        de = ldEnergyGrid[k] - ldEnergyGrid[k - 1];
        temperatures[i][j][k] = (-log(levelDensities[i][j][k + 2]) +
                                 8.0 * log(levelDensities[i][j][k + 1]) -
                                 8.0 * log(levelDensities[i][j][k - 1]) +
                                 log(levelDensities[i][j][k - 2])) /
                                (12. * de);
      ...
      de = ldEnergyGrid[1] - ldEnergyGrid[0];
      temperatures[i][j][0] = (-25.0 * log(levelDensities[i][j][0]) +
                               48.0 * log(levelDensities[i][j][1]) -
                               36.0 * log(levelDensities[i][j][2]) +
                               16.0 * log(levelDensities[i][j][3]) -
                               3.0 * log(levelDensities[i][j][4])) /
                              (12.0 * de);

      de = ldEnergyGrid[2] - ldEnergyGrid[1];
      temperatures[i][j][1] = (-25.0 * log(levelDensities[i][j][1]) +
                               48.0 * log(levelDensities[i][j][2]) -
                               36.0 * log(levelDensities[i][j][3]) +
                               16.0 * log(levelDensities[i][j][4]) -
                               3.0 * log(levelDensities[i][j][5])) /
                              (12.0 * de);

      de = ldEnergyGrid[NUME - 2] - ldEnergyGrid[NUME - 3];
      temperatures[i][j][NUME - 2] =
          (25.0 * log(levelDensities[i][j][NUME - 2]) -
           48.0 * log(levelDensities[i][j][NUME - 3]) +
           36.0 * log(levelDensities[i][j][NUME - 4]) -
           16.0 * log(levelDensities[i][j][NUME - 5]) +
           3.0 * log(levelDensities[i][j][NUME - 6])) /
          (12.0 * de);

      de = ldEnergyGrid[NUME - 1] - ldEnergyGrid[NUME - 2];
      temperatures[i][j][NUME - 1] =
          (25.0 * log(levelDensities[i][j][NUME - 1]) -
           48.0 * log(levelDensities[i][j][NUME - 2]) +
           36.0 * log(levelDensities[i][j][NUME - 3]) -
           16.0 * log(levelDensities[i][j][NUME - 4]) +
           3.0 * log(levelDensities[i][j][NUME - 5])) /
          (12.0 * de);
 ...
 ******************************************************************************/
ZDistribution::ZDistribution(double *Z0, double *sZ0, double *FZZ0,
                             double *FNZ0) {
  ...
  // Peak parameters
  sigz140_Lc.push_back(0.566);
  sigz140_Lc.push_back(0.0);
  sigz140_Lc.push_back(0.0064);
  sigz140_Lc.push_back(0.0109);
  sigz140_Lc.push_back(0.0);
  delz140_Lc.push_back(-0.487);
  delz140_Lc.push_back(0.0);
  delz140_Lc.push_back(0.0180);
  delz140_Lc.push_back(0.0);
  delz140_Lc.push_back(-0.00203);

  // Legacy CGMF values
  Fz140_Lc.push_back(1.242);
  Fz140_Lc.push_back(0.0);
  Fz140_Lc.push_back(-0.0183);
  Fz140_Lc.push_back(-0.0152);
  Fz140_Lc.push_back(0.0);
  ...
  Fn140_Lc.push_back(1.076);
  Fn140_Lc.push_back(0.0);
  Fn140_Lc.push_back(0.0);
  Fn140_Lc.push_back(0.0);
  Fn140_Lc.push_back(0.0);
  sigzSL_Lc.push_back(-0.0038);
  sigzSL_Lc.push_back(0.0);
  sigzSL_Lc.push_back(0.0);
  sigzSL_Lc.push_back(0.0);
  sigzSL_Lc.push_back(0.0);
  delzSL_Lc.push_back(-0.0080);
  delzSL_Lc.push_back(0.0);
  delzSL_Lc.push_back(0.0);
  delzSL_Lc.push_back(0.0);
  delzSL_Lc.push_back(0.0);
  FzSL_Lc.push_back(0.0030);
  FzSL_Lc.push_back(0.0);
  FzSL_Lc.push_back(0.0);
  FzSL_Lc.push_back(0.0);
  FzSL_Lc.push_back(0.0);
  FnSL_Lc.push_back(-0.0006);
  FnSL_Lc.push_back(0.0);
  FnSL_Lc.push_back(0.0);
  FnSL_Lc.push_back(0.0);
  FnSL_Lc.push_back(0.0);

  // Symmetry parameters
  SL50_Lc.push_back(0.191);
  SL50_Lc.push_back(0.0);
  SL50_Lc.push_back(-0.0076);
  SL50_Lc.push_back(0.0);
  SL50_Lc.push_back(0.0);
  sigz50_Lc.push_back(0.356);
  sigz50_Lc.push_back(0.060);
  sigz50_Lc.push_back(0.0);
  sigz50_Lc.push_back(0.0);
  sigz50_Lc.push_back(0.0);
  delzmax_Lc.push_back(0.699);
  delzmax_Lc.push_back(0.0);
  delzmax_Lc.push_back(0.0);
  delzmax_Lc.push_back(0.0);
  delzmax_Lc.push_back(0.0);

  // Wing parameters
  sigzSLW_Lc.push_back(-0.045);
  sigzSLW_Lc.push_back(0.0094);
  sigzSLW_Lc.push_back(0.0);
  sigzSLW_Lc.push_back(0.0);
  sigzSLW_Lc.push_back(0.0);
  delzSLW_Lc.push_back(0.0);
  delzSLW_Lc.push_back(-0.0045);
  delzSLW_Lc.push_back(0.0);
  delzSLW_Lc.push_back(0.0);
  delzSLW_Lc.push_back(0.0);
  FzSLW_Lc.push_back(0.159);
  FzSLW_Lc.push_back(-0.028);
  FzSLW_Lc.push_back(0.0);
  FzSLW_Lc.push_back(0.0);
  FzSLW_Lc.push_back(0.0);
  FnSLW_Lc.push_back(0.039);
  FnSLW_Lc.push_back(0.0);
  FnSLW_Lc.push_back(0.0);
  FnSLW_Lc.push_back(0.0);
  FnSLW_Lc.push_back(0.0);
  ...
  // Peak parameters
  sigz140_Hc.push_back(0.542);
  sigz140_Hc.push_back(1.310);
  sigz140_Hc.push_back(0.033);
  sigz140_Hc.push_back(0.0);
  sigz140_Hc.push_back(-0.005);
  delz140_Hc.push_back(-0.428);
  delz140_Hc.push_back(0.0);
  delz140_Hc.push_back(0.0);
  delz140_Hc.push_back(0.164);
  delz140_Hc.push_back(-0.0116);
  Fz140_Hc.push_back(1.0);
  Fz140_Hc.push_back(0.0);
  Fz140_Hc.push_back(0.0);
  Fz140_Hc.push_back(0.0);
  Fz140_Hc.push_back(0.0);
  Fn140_Hc.push_back(1.0);
  Fn140_Hc.push_back(0.0);
  Fn140_Hc.push_back(0.0);
  Fn140_Hc.push_back(0.0);
  Fn140_Hc.push_back(0.0);
  sigzSL_Hc.push_back(0.0);
  sigzSL_Hc.push_back(0.0);
  sigzSL_Hc.push_back(0.0);
  sigzSL_Hc.push_back(0.0);
  sigzSL_Hc.push_back(0.0);
  delzSL_Hc.push_back(0.0);
  delzSL_Hc.push_back(0.0);
  delzSL_Hc.push_back(0.0);
  delzSL_Hc.push_back(0.0);
  delzSL_Hc.push_back(0.0);
  FzSL_Hc.push_back(0.0);
  FzSL_Hc.push_back(0.0);
  FzSL_Hc.push_back(0.0);
  FzSL_Hc.push_back(0.0);
  FzSL_Hc.push_back(0.0);
  FnSL_Hc.push_back(0.0);
  FnSL_Hc.push_back(0.0);
  FnSL_Hc.push_back(0.0);
  FnSL_Hc.push_back(0.0);
  FnSL_Hc.push_back(0.0);
  ...
  // Symmetry parameters
  SL50_Hc.push_back(0.191);
  SL50_Hc.push_back(0.0);
  SL50_Hc.push_back(-0.0076);
  SL50_Hc.push_back(0.0);
  SL50_Hc.push_back(0.0);
  sigz50_Hc.push_back(0.542);
  sigz50_Hc.push_back(1.310);
  sigz50_Hc.push_back(0.033);
  sigz50_Hc.push_back(0.0);
  sigz50_Hc.push_back(-0.005);
  delzmax_Hc.push_back(0.0);
  delzmax_Hc.push_back(0.0);
  delzmax_Hc.push_back(0.0);
  delzmax_Hc.push_back(0.0);
  delzmax_Hc.push_back(0.0);
  ...
  // Wing parameters
  sigzSLW_Hc.push_back(0.0);
  sigzSLW_Hc.push_back(0.0);
  sigzSLW_Hc.push_back(0.0);
  sigzSLW_Hc.push_back(0.0);
  sigzSLW_Hc.push_back(0.0);
  delzSLW_Hc.push_back(0.0);
  delzSLW_Hc.push_back(0.0);
  delzSLW_Hc.push_back(0.0);
  delzSLW_Hc.push_back(0.0);
  delzSLW_Hc.push_back(0.0);
  FzSLW_Hc.push_back(0.0);
  FzSLW_Hc.push_back(0.0);
  FzSLW_Hc.push_back(0.0);
  FzSLW_Hc.push_back(0.0);
  FzSLW_Hc.push_back(0.0);
  FnSLW_Hc.push_back(0.0);
  FnSLW_Hc.push_back(0.0);
  FnSLW_Hc.push_back(0.0);
  FnSLW_Hc.push_back(0.0);
  FnSLW_Hc.push_back(0.0);
...
void ZDistribution::ComputeZpParams(int Zf, int Af, double PE) {
  ...
  if ((PE >= PE_L) && (PE <= PE_H)) {
    ...
    // In LegacyCGMF, there is no linear interpolation, just a sharp change to
    // intermediate energy treatment
 ...
 ******************************************************************************/
double ZDistribution::WahlEq17(int Zf, int Af, double PE,
  ...
  // Eq. 17
  par = coeffs[0] + coeffs[1] * (Zfd - 92.) + coeffs[2] * (Afd - 236.) +
        coeffs[3] * (PE - 6.551) + coeffs[4] * (Afd - 236.) * (Afd - 236.);
  return par;
}
 ...
 * whatever parameter we are calculating.
 ******************************************************************************/
double ZDistribution::WahlEq19(int Zf, int Af, double PE,
                               vector<double> coeffs) {
...
void ZDistribution::ComputeMassBounds(int Zf, int Af, double PE) {
  ...
  Ba = dAf - Amax;
  Bb = Amax;
  /* -- This is the formula seen in Wahl's paper, but in his code cyfp.f90 it is
  the above - the paper appears to be incorrect Ba = Amax; Bb = dAf - Amax;
  */

  /* To match the legacy CGMF, we had flipped delz140, but the mass bounds
   * requires the original delz140 */
  double delz140_tmp = delz140;
  if (Method == "LegacyCGMF" && PE >= PE_H) {
    delz140_tmp = -delz140;
  }

  B4 = (delzmax - delz140_tmp + Amax * (SL50) + 140.0 * delzSL) /
       (SL50 + delzSL);
  B3 = dAf - B4;
  B5 = dAf - B2;
  B6 = dAf - B1;
  ...
  iBa = int(Af - Amax + 0.5);
  iBb = int(Amax + 0.5);
  iB4 = (int)floor((delzmax - delz140_tmp + Amax * SL50 + 140.0 * delzSL) /
                       (SL50 + delzSL) +
                   0.5);
  iB3 = Af - iB4;
  iB5 = Af - iB2;
  ...
  // Check boundaries
  if ((B1 < B2) && (B2 < B3) && (B3 < Ba) && (Ba < Bb) && (Bb < B4) &&
      (B4 < B5) && (B5 < B6)) {
    dAf = dAf;
  } else {
    printf(" -- WARNING: Wahl mass bounds have issues: %.1f %.1f %.1f %.1f "
           "%.1f %.1f %.1f %.1f\n",
...
void ZDistribution::PeakRegion(int Amin, int Amax, int Af) {
  ...
  for (int A = Amin; A <= Amax; A++) {
    ...
    if ((A >= iB2) && (A <= iB3)) {
      ...
      Fn[A] =
          Fn140 -
          FzSL * (A - (Af - 140)); // Legacy CGMF calculations used FzSL as FnSL
    } else if ((A >= iB4) && (A <= iB5)) {
    ...
    if ((Adbl >= B2) && (Adbl <= B3)) {
      ...
      Fn[A] = Fn140 +
              FnSL * (Adbl - 140.); // While not explicitly given, FnSL is
                                    // provided in the footnote (b) of Tab. 2
    }
  }
  return;
}
...
/*******************************************************************************
 * Computes the Wahl Zp parameters (delz,sigz,Fz,Fn) in the symmetry region (Eq.
 *12)
 *------------------------------------------------------------------------------
 * Loops through the Amin and Amax values, computing the Wahl Zp parameters in
...
/*******************************************************************************
 * Computes the Wahl Zp parameters (delz,sigz,Fz,Fn) in the far wing region (Eq.
 *14)
 *------------------------------------------------------------------------------
 * Loops through the Amin and Amax values, computing the Wahl Zp parameters in
 ...
 ******************************************************************************/
void ZDistribution::ComputeZDistribution(int Zf, int Af, double PE, int Amin,
                                         int Amax, int dZ, double *Z0,
                                         double *sZ0, double *FZZ0,
                                         double *FNZ0, int *Zmin, int *Zmax) {
  ...
  // Calculate the mean charge Z0, width sZ0, and Z/N even-odd effects FZZ0,
  // FNZ0 for each mass
 ...
 ******************************************************************************/
string ZDistribution::ReturnMethod() { return Method; }
...
// TODO: PROBABLY SHOULD ALSO RECODE THE BELOW TO MAKE SURE IT FOLLOWS A AND A'
// NOTATION (P.J.)
// -----------------------------------------------------------------------------------------------------------------------------
/*******************************************************************************
 * computeWahlParametersForChargeDistribution
...
void FissionFragments::computeWahlParametersForChargeDistribution(void) {
  ...
  if (PE <= 8.0) {
    ...
    sigz140t = sigz140[0] + sigz140[1] * (Zt - 92) + sigz140[2] * (Ac - 236) +
               sigz140[3] * (PE - 6.551) + sigz140[4] * (Ac - 236) * (Ac - 236);
    delz140t = delz140[0] + delz140[1] * (Zt - 92) + delz140[2] * (Ac - 236) +
               delz140[3] * (PE - 6.551) + delz140[4] * (Ac - 236) * (Ac - 236);
    Fz140t = Fz140[0] + Fz140[1] * (Zt - 92) + Fz140[2] * (Ac - 236) +
             Fz140[3] * (PE - 6.551) + Fz140[4] * (Ac - 236) * (Ac - 236);
    Fn140t = Fn140[0] + Fn140[1] * (Zt - 92) + Fn140[2] * (Ac - 236) +
             Fn140[3] * (PE - 6.551) + Fn140[4] * (Ac - 236) * (Ac - 236);
    sigzSLt = sigzSL[0] + sigzSL[1] * (Zt - 92) + sigzSL[2] * (Ac - 236) +
              sigzSL[3] * (PE - 6.551) + sigzSL[4] * (Ac - 236) * (Ac - 236);
    delzSLt = delzSL[0] + delzSL[1] * (Zt - 92) + delzSL[2] * (Ac - 236) +
              delzSL[3] * (PE - 6.551) + delzSL[4] * (Ac - 236) * (Ac - 236);
    FzSLt = FzSL[0] + FzSL[1] * (Zt - 92) + FzSL[2] * (Ac - 236) +
            FzSL[3] * (PE - 6.551) + FzSL[4] * (Ac - 236) * (Ac - 236);
    SL50t = SL50[0] + SL50[1] * (Zt - 92) + SL50[2] * (Ac - 236) +
            SL50[3] * (PE - 6.551) + SL50[4] * (Ac - 236) * (Ac - 236);
    sigz50t = sigz50[0] + sigz50[1] * (Zt - 92) + sigz50[2] * (Ac - 236) +
              sigz50[3] * (PE - 6.551) + sigz50[4] * (Ac - 236) * (Ac - 236);
    delzmaxt = delzmax[0] + delzmax[1] * (Zt - 92) + delzmax[2] * (Ac - 236) +
               delzmax[3] * (PE - 6.551) + delzmax[4] * (Ac - 236) * (Ac - 236);
    sigzSLWt = sigzSLW[0] + sigzSLW[1] * (Zt - 92) + sigzSLW[2] * (Ac - 236) +
               sigzSLW[3] * (PE - 6.551) + sigzSLW[4] * (Ac - 236) * (Ac - 236);
    delzSLWt = delzSLW[0] + delzSLW[1] * (Zt - 92) + delzSLW[2] * (Ac - 236) +
               delzSLW[3] * (PE - 6.551) + delzSLW[4] * (Ac - 236) * (Ac - 236);
    FzSLWt = FzSLW[0] + FzSLW[1] * (Zt - 92) + FzSLW[2] * (Ac - 236) +
             FzSLW[3] * (PE - 6.551) + FzSLW[4] * (Ac - 236) * (Ac - 236);
    FnSLWt = FnSLW[0] + FnSLW[1] * (Zt - 92) + FnSLW[2] * (Ac - 236) +
             FnSLW[3] * (PE - 6.551) + FnSLW[4] * (Ac - 236) * (Ac - 236);

  } else if (PE <= 30.) { // I Stetcu: this was extended to 30 MeV (see pages 22
                          // and 25 for PE>~20 MeV)
    ...
    sigz140t = (sigz140[0] + sigz140[2] * (Zt - 92)) +
               ((sigz140[1] + sigz140[3] * (Zt - 92)) -
                (sigz140[0] + sigz140[2] * (Zt - 92))) *
                   (1.0 - exp(-sigz140[4] * PE));
    delz140t = (delz140[0] + delz140[2] * (Zt - 92)) +
               ((delz140[1] + delz140[3] * (Zt - 92)) -
                (delz140[0] + delz140[2] * (Zt - 92))) *
                   (1.0 - exp(-delz140[4] * PE));
    Fz140t = 1.0;
    Fn140t = 1.0;
    sigzSLt = 0.0;
    delzSLt = 0.0;
    ...
    SL50t =
        (SL50[0] + SL50[2] * (Zt - 92)) +
        ((SL50[1] + SL50[3] * (Zt - 92)) - (SL50[0] + SL50[2] * (Zt - 92))) *
            (1.0 - exp(-SL50[4] * PE));
    sigz50t = (sigz50[0] + sigz50[2] * (Zt - 92)) +
              ((sigz50[1] + sigz50[3] * (Zt - 92)) -
               (sigz50[0] + sigz50[2] * (Zt - 92))) *
                  (1.0 - exp(-sigz50[4] * PE));
    delzmaxt = 0.0;
    sigzSLWt = 0.0;
    delzSLWt = 0.0;
    ...
    cerr << "[computeWahlParameters] Wahl parameters only defined up to "
            "Einc+Sn=20 MeV!"
         << endl;
    exit(-1);
  }
  ...
  int B4 = (int)floor((delzmaxt - delz140t + Apmax * SL50t + 140 * delzSLt) /
                          (SL50t + delzSLt) +
                      0.5); // nint() in F95
  int B3 = (Ac - B4);
  int B5 = (Ac - B2);
...
void readDeformations(void) {
  ...
  if (!fp)
    cgmTerminateCode("deformtions data file not found");
  ...
  while (getline(fp, str)) {
    if (str[0] == '#')
      continue;
    Z = atoi(str.substr(0, 4).c_str());
    if (Z > 99)
      break;
    A = atoi(str.substr(4, 8).c_str());
    if (A > 199)
      continue;
    str = str.substr(44);
    if (str[6] != ' ')
      beta2[1000 * Z + A] = atof(str.c_str());
  }
...
void readTemperatures(void) {
  ...
  if (!fp)
    cgmTerminateCode("temperatures data file not found");
...
double getTemperature(int zaid, double energy) {
  ...
  if (temperatures[zaid][0] == 0.0) {
    ...
    for (int i = 0; i < NUM_GRID; i++)
      temperatures[zaid][i] = 5.0; // Default if missing
...
void readSpinScaling(void) {
  ...
  if (!fp)
    cgmTerminateCode("spin-scaling model file not found");

  while (getline(fp, line)) {
    if (line[0] == '#' or line == "")
      continue;
    istringstream(line) >> alpha_coefficients[NumReactions][0] >>
        alpha_coefficients[NumReactions][1] >>
        alpha_coefficients[NumReactions][2];
    NumReactions++;
    if (NumReactions > ALPHA_MAX_ZAIDc)
      cgmTerminateCode(
          "Need to increase the size of alpha_coefficients array\n");
  }
...
void readLDP(void) {
  ...
  if (!fp)
    cgmTerminateCode("level density parameters data file not found");
...
void FissionFragments::readNeutronBindingEnergies(void) {
  ...
  if (!dataFile) {
    cerr
        << "ERROR: Cannot find neutron separation energies from Audi 2003 file!"
        << endl;
    exit(-1);
  }
  ...
  while (!dataFile.eof()) { // Loop through file
    ...
    if (str.substr(0, 1) != "#" &&
      ...
      if (string1.find("*") ==
          string::npos) { // Table contains "*" when there is no exp. value
...
void readPreEqAngularDistributionParameters(void) {
  ...
  /*
     ...
     for(int i=0; i<4;i++){
        fp >> preeqScatteringParams[0][i][0] >> preeqScatteringParams[0][i][1]
     >> preeqScatteringParams[0][i][2] >> preeqScatteringParams[0][i][3] >>
     preeqScatteringParams[0][i][4] >> preeqScatteringParams[0][i][5];
     }
     for(int i=0; i<4;i++){
        fp >> preeqScatteringParams[1][i][0] >> preeqScatteringParams[1][i][1]
     >> preeqScatteringParams[1][i][2] >> preeqScatteringParams[1][i][3] >>
     preeqScatteringParams[1][i][4] >> preeqScatteringParams[1][i][5];
     }
...
double getMassExcess(int zaid) {
  if (masses[zaid] == 0.0) {
    stringstream zaid_s;
    zaid_s << zaid;
    cgmTerminateCode("Mass not found in mass table for this nucleus: " +
                     zaid_s.str());
  }
  return masses[zaid];
}
...
void FissionFragments::readYieldsATKE(string yieldsFilename) {
  ...
  if (!yieldsFile) {
    cerr << "ERROR: Cannot find fission fragment yields file: "
         << yieldsFilename << endl;
    exit(-1);
  }
  ...
  Amax = Amax + 5;
  norm = 0.0; // TODO: I THINK WE NEED TO USE THE NORMALIZATION FROM THE
              // SYMMETRIZED YIELDS (P.J.)
  ...
  for (int i = 0; i < NUMA; i++) {
    ...
    for (int j = 0; j < NUMTKE - 1; j++)
      avTKE__ += 0.5 * (YATKE[i][j] + YATKE[i][j + 1]) * (j + 0.5); // Integrate
  }
...
void FissionFragments::readYieldsAZTKE(string yieldsFilename) {
  ...
  if (!yieldsFile) {
    cerr << "ERROR: Cannot find fission fragment yields [A,Z,TKE,Y] file: "
         << yieldsFilename << endl;
    exit(-1);
  }
  ...
  while (!yieldsFile.eof()) { // Loop through the yields file and input the
                              // necessary info
    if (c[0] == '#') {
      yieldsFile.getline(c, 299);
      continue;
  ...
  yieldsFile.close();
  // norm = 0.0; // TODO: I THINK WE NEED TO USE THE NORMALIZATION FROM THE
  // SYMMETRIZED YIELDS (P.J.) symmetrize yields, if needed
  for (int i = Asym; i < Amax; i++) {
    for (int j = 0; j < NUMTKE; j++) {
      YATKE[i][j] = YATKE[Ac - i][j];
      // norm += YATKE[i][j];
...
double FissionFragments::spinParameter2(double U, int A, int Z) {
  ...
  double momInertia = 0.4 * pow((double)A, 5. / 3.) * 1.2 * 1.2 * .5 *
                      (NEUTRONMASS + PROTONMASS) * amuMeV;
  momInertia *=
      (1.0 + 0.31 * beta2[Z * 1000 + A] + 0.44 * pow(beta2[Z * 1000 + A], 2.));
...
void FissionFragments::constructYields(Yields *ffy) {
  ...
  // Initialize to the original compound system values -- these are the ones
  ...
  double excitEnergy =
      ncl[0]
          .max_energy; // Maximum energy that is available for neutron emission
  ...
  if (!sf_flag) {
    ...
    incidentEnergy =
        excitEnergy -
        SnCompound; // Calculate the "effective" incident neutron energy
  }

  double WahlPE = incidentEnergy0 + SnCompound0;

  // Set the spin strength (i.e. alphaI) for this new compound nucleus and
  // "effective" incident neutron energy
  setSpinStrength(ZAIDc - n_pfn, incidentEnergy);
  //    setSpinStrength(incidentEnergy); // IS parameterization

  // Set the fissioning system for this new compound nucleus and "effective"
  ...
  for (int a = Amin; a <= Amax; a++) {
    YA[a] /= s;
    if (YA[a] > maxYieldA)
      maxYieldA = YA[a]; // Find the new maxY(A)
  }

  // Rescale the <TKE> for this new compound nucleus and "effective" incident
  // neutron energy
  ...
  if (WahlParams.ReturnMethod() == "LegacyCGMF") {
    ...
    WahlParams.ComputeZDistribution(Zc0, Ac0, incidentEnergy0 + SnCompound0,
                                    Amin, Amax, dZ, Z0, sZ0, FZZ0, FNZ0, &Zmin,
                                    &Zmax);
  }
  // computeWahlParametersForChargeDistribution(); // Compute the new Wahl
...
//     numZAID++;
//     if (numZAID>MAX_ZAIDc) cgmTerminateCode("Increase size of multi-chance
//     fission array\n");
//   }
...
void FissionFragments::readMultichanceFissionData(void) {
  ...
  // TODO: RECODE THIS SO IT DOESN'T HANG IF THE MULTICHANCEFISSION.DAT FILE HAS
  // THE INCORRECT FORMAT -- MIGHT NEED TO RESTRUCTURE THE FILE TOO
  // -----------------------------------------------------------------------------------------------------------------------------
  ...
  if (!fissdata.is_open()) {
    cerr << "[readMultichanceFissionData] file " << fileFissionData
         << " was not found" << endl;
    exit(-1);
  }
  ...
  bool found = false; // Tags when we've determined the multi-chance fission
                      // probabilities for this incident particle energy
  double **Pf;
  double *energies;
  ...
  while (!fissdata.eof()) {
    ...
    if (Nparam < 3) {
      fissdata.getline(c, 399);
      continue;
    } // Ignore empty lines (or those with less than 3 items)
    ...
    if (ZAIDc - 1 == id) {          // Neutron-induced fission
      ...
      emissprob = new double[nn + 1]; // Array for final emission probabilities
                                      // (at En = incidentEnergy)
      ...
      if (incidentEnergy < energies[0] ||
          incidentEnergy > energies[nenergies - 1]) {
        cerr << "[readMultichanceFissionData] could not find multi-chance "
                "fission data for this incident energy"
             << endl;
        exit(-1);
      }

      // Find the multi-chance fission probabilities at the specified incident
      // particle energy
      for (int i = 0; i < nenergies; i++) {
        if (incidentEnergy <= energies[i]) {
          ...
          for (int j = 0; j <= nn; j++) {
            ...
            if (slope != 0.0) {
              emissprob[j] =
                  (incidentEnergy - energies[i - 1]) * slope + Pf[i - 1][j];
            } else {
              emissprob[j] = Pf[i][j];
            }
          }
      ...
      // PT, April 2021: segmentation fault if nemit>=nn+1... simple FIX for
      // now: replace <=nemit with <=min(nemit,nn)

      // Determine the maximum number of pre-fission neutrons
      int k = nn;
      //      for (int j=0;j<=nemit;j++) { // TODO: If "statTotalNeutrons"
      //      returns a value lower than that in the multi-chance file, we'll
      //      only use the "statTotalNeutrons" value.
      for (int j = 0; j <= min(nemit, nn);
           j++) { // TODO: If "statTotalNeutrons" returns a value lower than
                  // that in the multi-chance file, we'll only use the
                  // "statTotalNeutrons" value.
        if (emissprob[j] <= 0.0) {
          k = j - 1;
          break;
        }
      ...
      for (int j = 0; j <= nemit; j++) {
        emissprob[j] /= s;
        if (emissprob[j] > emissprob_max)
          emissprob_max = emissprob[j];
      }
      found = true;
      break;
  ...
  // If we didn't find the multi-chance fission probabilities and we need it,
  // then exit
  if (!found && nemit > 0) {
    cerr << "[readMultichanceFissionData] No multichance fission probabilities "
            "for this nucleus!"
         << endl;
    exit(-1);
  }
...
void FissionFragments::getPrefissionNeutronEnergy(double *excitEnergy) {
  ...
  // Can't sample negative energies -- i.e. we're below the barrier of the
  // 2nd-chance daughter
  if (eps_max < 0.)
    preeq_flag = false;
  if (eps_max < 0.)
  ...
  // Sample the first pre-fission neutron from the spectrum sp1.dat (either
  // evaporation or pre-fission)
  double max_prob = 0.;
  for (int i = 0; i < kfmax; i++) {
    if (sp1[i] > max_prob)
      max_prob = sp1[i]; // Update the maximum probability for the spectrum sp1
  }

  // Sample the first neutron-out spectrum 'sp1' (contains evaporation or
  ...
  cmEnergy_pfn[0] =
      energy_grid1[kf] +
      0.5 * ENERGY_BIN *
          (1 - 2 * rng_cgm()); // Continuum-to-continuum energy transfer

  // if the neutron was pre-equilibrium, construct the CDF for the angular
  // sampling
  ...
  if (preeq_flag) {
    ...
    p0 =
        aparm + bparm * Einc + cparm * Einc * Einc + dparm * Einc * Einc * Einc;
    aparm = 0.;
    bparm = 0.;
    cparm = 0.;
    dparm = 0.;
  ...
  for (int i = 2; i <= n; ++i) {
    ...
    if (eps_max < 0.) { // << 1.0.6 >> Can this happen? ...
      cout << i << " nemit_now=" << n << " *eps_max=" << eps_max
           << " kstart=" << kstart << endl;
    }

    if (eps_max < ENERGY_BIN) {    // IS: added a trap to catch zero energy
                                   // prefission neutrons
    ...
    if (kf < 0)
      cout << i << " nemit_now=" << n << " kstart=" << kstart << " kf=" << kf
           << endl;
 ...
 ******************************************************************************/
void FissionFragments::getSpinScalingCoefficients(int ZAID_0, bool sf_flag,
                                                  double *alpha_0,
                                                  double *alpha_slope) {
 ...
 * neutron energy einc and the compound nucleus ZAID_0. TODO: PUT THIS
 *INFORMATION IN A DATA FILE (P.J.)
 ******************************************************************************/
...
void FissionFragments::setSpinStrength(double einc) {
  ...
  case 92239:
    ...
    if(einc<1.5)
      ...
      alphaI=2.49-7.11*exp(-1.28*einc); // this would be negative at around 0.8,
    keep alphaI constant below 1.5 MeV break;
    */
...
void readPreeqData(void) {
  ...
  if (!fp)
    cgmTerminateCode("preequilibrium data file not found");
  ...
  for (int i = 0; i < n_reactions; i++)
    fp >> preeq_id[i] >> preeq_params[i][0] >> preeq_params[i][1] >>
        preeq_params[i][2] >> preeq_params[i][3];
