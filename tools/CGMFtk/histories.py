...
class Histories:
    def __init__(
        self, filename=None, nevents=None, ang_mom_printed=False, from_arr=None
    ):
        ...
        if from_arr is not None:
            ...
            if shape[1] == 26:
                ...
                print(
                    "Shape should be (nevents,c), where c can be 25 or 26 "
                    "(depending on whether or not angular momentum is printed)"
                )
                exit(1)
        else:
        ...
        # -- just fragments (no pre-fission neutrons)
        self.nEcmFragments = self.nEcm
        self.nElabFragments = self.nElab
    ...
    def _readHistoryFileFromCGMF(
            self, filename, nevents, ang_mom_printed=False):
        """
        Reads CGMF history file (filename) and returns a
        numpy array of lists for each field
        """
        ...
        # -- neutron directional cosines in center-of-mass frame of the fragment
        cmNeutronDircos = ([])
        ...
        # -- pre-fission neutron energies in LAB frame (MeV)
        labPreFissionNeutronEnergies = ([])
        # -- pre-fission neutron directional cosines in LAB frame
        labPreFissionNeutronDircos = ([])
        ...
        if line.find("#") != 0:
            ...
            if len(data) > 2:
                self.time = float(
                    data[-1]
                )  # timing cut-off for gammas, -1 records all times of emission
            else:
                self.time = 0
        ...
        while True:
            ...
            if nn > 0:
                ...
                for i in range(nn):
                    cmDn.append(
                        np.array(
                            [
                                float(data[0 + i * num_dptns]),
                                float(data[1 + i * num_dptns]),
                                float(data[2 + i * num_dptns]),
                            ]
                        )
                    )
            ...
            # -- read photon momentum vectors
            # -- (lab=center-of-mass frame in this case, for now)
            Dg = []
            Eg = []
            Tg = []

            if ng > 0 and self.time < 0.0:
                data = f.readline().split()
                for i in range(ng):
                    Dg.append(
                        np.array(
        ...
        if ang_mom_printed:
            fields = fields + (cmNeutronDeltaJ,)

        fields = tuple([np.array(x, dtype=object) for x in fields])
    ...
    def getGammaEcm(self):
        """Returns a list of CM gamma energies for each fission fragment"""
        return self.gEcm
        ...
        for i in range(len(nLF)):
            for j in range(nLF[i]):
                for k in range(j, nLF[i]):
                    tempcos = (
                        ncos[c][j][0] * ncos[c][k][0]
                        + ncos[c][j][1] * ncos[c][k][1]
                        + ncos[c][j][2] * ncos[c][k][2]
                    )
    ...
    def gammaSpec(self, Egamma, dEgamma, post=True):
        ...
        if np.any(eglist < 0.0) or np.any(dEgamma < 0.0):
        ...
        for n, gammas in enumerate(self.gElab):
            ...
            for m, eg in enumerate(eglist):
                ...
                if np.any(
                    [g <= eg + deglist[m] and g >= eg - deglist[m] for g in gammas]
                ):
                    log[m] = True
    ...
    def gammaMultiplicity(
        self, Eth=None, Afragment=None, Zfragment=None, minTime=None, maxTime=None
    ):
    ...
    def summaryTable(self):
        ...
        table.append(
            [
                "A",
                "{0:5.2f}".format(np.mean(self.A)),
                "{0:5.2f}".format(np.mean(self.Al)),
                "{0:5.2f}".format(np.mean(self.Ah)),
            ]
        )
        table.append(
            [
                "Z",
                "{0:5.2f}".format(np.mean(self.Z)),
                "{0:5.2f}".format(np.mean(self.Zl)),
                "{0:5.2f}".format(np.mean(self.Zh)),
            ]
        )
        table.append(
            [
                "TXE / U (MeV)",
                "{0:5.2f}".format(np.mean(self.TXE)),
                "{0:5.2f}".format(np.mean(self.Ul)),
                "{0:5.2f}".format(np.mean(self.Uh)),
            ]
        )
        table.append(
            [
                "TKE / KE (MeV)",
                "{0:5.2f}".format(np.mean(self.TKEpre)),
                "{0:5.2f}".format(np.mean(self.KEl)),
                "{0:5.2f}".format(np.mean(self.KEh)),
            ]
        )
        table.append(
            [
                "J ($\hbar$)",
                "{0:5.2f}".format(np.mean(self.J)),
                "{0:5.2f}".format(np.mean(self.Jl)),
                "{0:5.2f}".format(np.mean(self.Jh)),
            ]
        )
        table.append(
            [
                "parity",
                "{0:5.2f}".format(np.mean(self.P)),
                "{0:5.2f}".format(np.mean(self.Pl)),
                "{0:5.2f}".format(np.mean(self.Ph)),
            ]
        )
        ...
        table.append(
            [
                r"$\langle \nu\rangle$",
                "{0:.3f}".format(np.mean(self.nuLF + self.nuHF)),
                "{0:.3f}".format(np.mean(self.nuLF)),
                "{0:.3f}".format(np.mean(self.nuHF)),
                "{0:.3f}".format(np.mean(self.preFissionNu)),
                "{0:.3f}".format(np.mean(self.nutot + self.preFissionNu)),
            ]
        )
        table.append(
            [
                r"$\langle \epsilon_n^{cm}\rangle$ (MeV)",
                "{0:.3f}".format(self.meanNeutronEcmFragments()),
                "{0:.3f}".format(self.meanNeutronEcmLF()),
                "{0:.3f}".format(self.meanNeutronEcmHF()),
            ]
        )
        table.append(
            [
                r"$\langle E_n^{lab}\rangle$ (MeV)",
                "{0:.3f}".format(self.meanNeutronElabFragments()),
                "{0:.3f}".format(self.meanNeutronElabLF()),
                "{0:.3f}".format(self.meanNeutronElabHF()),
                "{0:.3f}".format(self.meanPreFissionNeutronElab()),
                "{0:.3f}".format(self.meanNeutronElab()),
            ]
        )
        ...
        table.append(
            [
                r"$\langle \nu_\gamma\rangle$",
                "{0:5.2f}".format(np.mean(self.nugtot)),
                "{0:5.2f}".format(np.mean(self.nugLF)),
                "{0:5.2f}".format(np.mean(self.nugHF)),
            ]
        )
        table.append(
            [
                r"$\langle E_\gamma^{lab}\rangle$ (MeV)",
                "{0:5.2f}".format(self.meanGammaElab()),
                "{0:5.2f}".format(self.meanGammaElabLF()),
                "{0:5.2f}".format(self.meanGammaElabHF()),
            ]
        )
...
def unnest_neutrons(df: pd.DataFrame):
    ...
    return df.explode(
        column=["neutron", "nEcm", "nElab", "nElabFrag", "nEcmFrag", "cmNdJ"]
    )
