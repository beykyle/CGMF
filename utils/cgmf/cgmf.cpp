...
int main(int argc, char *argv[]) {
  ...
  if (nevents < 0) {
    if (ip == 0) {
#ifdef MPIRUN
      cout << "Not implemented: Fission fragment yields calculation (negative "
              "-nevents option) will run with a single MPI rank.\n";
#endif
      rng.set_seed(startingEvent);
      set_rng(rng);
      yields = new cgmfYields(ZAIDt, incidentEnergy, -nevents, outfilename);
      printf("\n/// CGMF-generated scission fragment yields "
             "Y(Z,A,KE,U,J,Pi,px,py,pz) saved in file %s ///\n",
             outfilename.c_str());
    }
  } else {
...
void recordEvent(cgmfEvent *event) {
  ...
  // Neutron energies emitted from light fragment, in center-of-mass and lab
  // frame (MeV)
  ...
  // Neutron energies emitted from heavy fragment, in center-of-mass and lab
  // frame (MeV)
  ...
  for (int j = 0; j < nuPre; j++)
    sumEnPre += event->getPreFissionNeutronEnergy(j);
  ...
  // Gamma energies (MeV)
  for (int j = 0; j < nugLF; j++)
    sumEgLF += event->getPhotonEnergy(j); // Light fragment
  for (int j = nugLF; j < nugLF + nugHF; j++)
    sumEgHF += event->getPhotonEnergy(j); // Heavy fragment
...
void printSummaryEvents(cgmfEvent *event) {
  ...
  if (ip == 0) {
    ...
    printf("Average Kinetic Energies: LF = %.2f MeV ; HF = %.2f MeV ; <TKE> = "
           "%.2f MeV\n",
           sumKELF / nevents, sumKEHF / nevents, sumTKE / nevents);

    printf("Average Excitation Energies: LF = %.2f MeV ; HF = %.2f MeV ; <TXE> "
           "= %.2f MeV\n\n",
           sumXELF / nevents, sumXEHF / nevents, sumTXE / nevents);

    printf("Average Fragment Spins: <J>_LF = %.2f hbar ; <J>_HF = %.2f hbar ; "
           "<J> = %.2f hbar\n",
           sumJLF / nevents, sumJHF / nevents, sumJ / nevents);

    printf("\n*** Prompt Fission Neutrons ***\n\n");

    printf("Multiplicities (n/f):  <nu>_LF = %.2f ; <nu>_HF = %.2f ; "
           "<nu>_prefission = %.2f ; <nu>_tot = %.2f \n",
           double(sumNuLF) / nevents, double(sumNuHF) / nevents,
           double(sumNuPre) / nevents, double(sumNuTot) / nevents);

    printf("c-o-m Energies:  <Ecm>_LF = %.2f MeV ; <Ecm>_HF = %.2f MeV ; "
           "<Ecm>_prefission = %.2f MeV ; <Ecm>_tot = %.2f MeV\n",
           sumEcmLF / max(sumNuLF, 1), sumEcmHF / max(sumNuHF, 1),
           sumEnPre / max(sumNuPre, 1), sumEcm / max(sumNuTot, 1));

    printf("Lab. Energies:   <Elab>_LF = %.2f MeV ; <Elab>_HF = %.2f MeV ; "
           "<Elab>_prefission = %.2f MeV ; <Elab>_tot = %.2f MeV\n",
           sumElabLF / max(sumNuLF, 1), sumElabHF / max(sumNuHF, 1),
           sumEnPre / max(sumNuPre, 1), sumElab / max(sumNuTot, 1));

    printf("\n*** Prompt Fission Gammas ***\n\n");

    printf("Multiplicities (g/f):  <nu_g>_LF = %.2f ; <nu_g>_HF = %.2f ; "
           "<nu_g>_tot = %.2f \n",
           double(sumNugLF) / nevents, double(sumNugHF) / nevents,
           double(sumNugTot) / nevents);

    printf("Gamma Energies:   <Eg>_LF = %.2f MeV ; <Eg>_HF = %.2f MeV ; "
           "<Eg>_tot = %.2f MeV\n",
           sumEgLF / max(sumNugLF, 1), sumEgHF / max(sumNugHF, 1),
           sumEg / max(sumNugTot, 1));
...
void readUserInput(int argc, char *argv[], int ip) {
  ...
  if (outfilename == "") {
    if (nevents > 0) {
      outfilename = "histories.cgmf";
    } else {
      outfilename = "yields.cgmf";
    }
  }
  outfilename += "." + sip.str();
  ...
  if (timeCoincidenceWindow < 0.) {
    if (timeCoincidenceWindow != -1.) {
      cerr << "The time coincidence window can only be -1 (infinite time "
              "coincidence window) or positive"
           << endl;
...
------------------------------------------------------------------------------
*/
void printEventToFile(FILE *fp, cgmfEvent *event,
                      double timeCoincidenceWindow) {
  ...
  fprintf(
      fp, " %i %i %.3f %.1f %i %.3f %.3f %i %i 0\n",
      event->getLightFragmentMass(), event->getLightFragmentCharge(),
      event->getLightFragmentExcitationEnergy(), event->getLightFragmentSpin(),
  ...
  fprintf(fp, " %.3f %.3f %.3f %.3f %.3f %.3f\n",
          ...
          event->getLightFragmentPreMomentumY(),
          event->getLightFragmentPreMomentumZ(),
          event->getLightFragmentPostMomentumX(),
          event->getLightFragmentPostMomentumY(),
  ...
  fprintf(fp, " %i %i %.3f %.1f %i %.3f %.3f %i %i %i\n",
          event->getHeavyFragmentMass(), event->getHeavyFragmentCharge(),
          event->getHeavyFragmentExcitationEnergy(),
          event->getHeavyFragmentSpin(), event->getHeavyFragmentParity(),
          event->getHeavyFragmentKineticEnergy(),
          event->getHeavyFragmentKineticEnergyPost(),
          event->getHeavyFragmentNeutronNu(), event->getHeavyFragmentPhotonNu(),
          nup);

  fprintf(fp, " %.3f %.3f %.3f %.3f %.3f %.3f\n",
          event->getHeavyFragmentPreMomentumX(),
          event->getHeavyFragmentPreMomentumY(),
          event->getHeavyFragmentPreMomentumZ(),
          event->getHeavyFragmentPostMomentumX(),
          event->getHeavyFragmentPostMomentumY(),
